...
| Versión | Fecha | Tipo de Versión | Cambios Clave |
|---------|------|--------------|-------------|
| [v1.38.0](#v1-38-0) | 2026-09-17 | Versión menor | Compatibilidad con catalán: el conjunto de idiomas admitidos del API crece a 15, aceptado en todos los puntos donde se acepta un código de idioma, incluida la configuración de espacio de trabajo y empresa, el idioma de correo por solicitud de firma, los términos del firmante, las plantillas de correo predeterminadas y el idioma de la declaración del sello de organización |
| [v1.37.1](#v1-37-1) | 2026-09-15 | Versión de parche | Destinatarios CC: aceptados en `PUT`/`PATCH` de plantillas y solicitudes de firma, devueltos en todas las listas de destinatarios y copiados desde la plantilla al crear una solicitud de firma a partir de una plantilla |
| [v1.37.0](#v1-37-0) | 2026-09-14 | Versión menor | Sellos de organización: endpoints del ciclo de vida del sello bajo `/seals`, `seal_participants[]` en plantillas y solicitudes de firma, intercambio de sello en solicitudes enviadas, siete nuevos eventos de webhook |
| [v1.36.0](#v1-36-0) | 2026-08-31 | Versión menor | Anchor tags: `add_white_background`; respuestas de error estructuradas (`code`, `errors[]`); `warnings[]` en las respuestas de create-and-send |
| [v1.35.0](#v1-35-0) | 2026-08-13 | Versión Menor | Copia de plantilla entre espacios de trabajo: nuevo endpoint `POST /templates/{id}/copy` que copia una plantilla a otro espacio de trabajo (requiere clave API protegida) |
| [v1.33.0](#v1-33-0) | 2026-08-03 | Versión Menor | Cinco nuevos idiomas — checo (`cs`), sueco (`sv`), neerlandés (`nl`), rumano (`ro`) y noruego bokmål (`nb`) — aceptados en todos los puntos donde se acepta un código de idioma: configuración de espacio de trabajo/empresa, idioma de correo por solicitud de firma, términos del firmante y plantillas de correo predeterminadas |
| [v1.32.0](#v1-32-0) | 2026-07-22 | Versión Menor | Tamaño de fuente explícito por campo: `fontSize` opcional (8-48 px) en `format_rules` para campos con texto, aplicado en la vista de firma y en el PDF sellado con reducción automática preservada; nueva propiedad `font_size` en anchor tags |
| [v1.31.0](#v1-31-0) | 2026-07-14 | Versión Menor | Idioma de correo opcional por solicitud de firma: el nuevo campo `language` en create-and-send y update define el idioma de todos los correos dirigidos al firmante (recurre al valor predeterminado del espacio de trabajo/empresa si se omite) |
| [v1.30.0](#v1-30-0) | 2026-07-08 | Versión Menor | Recuperación de marcas del firmante: imágenes de firma / iniciales / sello (PNG en base64) y descarga de archivos subidos por el firmante, por firmante, a través de la API |
| [v1.29.0](#v1-29-0) | 2026-07-06 | Versión Menor | Subida de documentos en dos pasos (`POST /documents`) para archivos grandes de hasta 50MB; nuevo campo `document_id` al crear y crear-y-enviar |
| [v1.28.0](#v1-28-0) | 2026-07-05 | Versión Menor | Ejemplos de código del SDK de TypeScript (`@firma-dev/sdk`) agregados a cada endpoint; los endpoints de solicitud de firma incrustada y el endpoint legado `/documents` se eliminaron de la referencia |
| [v1.27.0](#v1-27-0) | 2026-06-29 | Versión de Parche | Esquemas de configuración documentados: las respuestas de solicitud de firma ahora exponen `show_qr_code`, `allow_presigning_download`, `identity_editable_fields`, `notify_identity_change_email`; las respuestas de plantilla exponen `show_qr_code`; `cancel` devuelve `emails_sent`; `resend` devuelve `recipients_failed`; nuevos esquemas `TemplateSettings` y `LegacyDocumentSettings` |
| [v1.26.0](#v1-26-0) | 2026-06-23 | Versión Menor | Configuración de espacio de trabajo: anulaciones de la etiqueta del botón de firma, interruptor de código QR, interruptor de descarga antes de firmar, interruptor de marca de agua parcial |
| [v1.25.2](#v1-25-2) | 2026-06-18 | Versión de Parche | Los campos proporcionados que no coinciden con ningún campo de la plantilla ahora devuelven `400` (anteriormente se ignoraban en silencio) en `POST /signing-requests` y `/create-and-send`; los valores de `type` de campo ahora se comparan sin distinguir mayúsculas y minúsculas |
| [v1.25.1](#v1-25-1) | 2026-06-14 | Versión de Parche | Los fallos del paso de envío en `create-and-send` ahora devuelven el estado real (`400`/`402`/`422`) con un `code` legible por máquina, en lugar de enmascararlos como `500` |
| [v1.25.0](#v1-25-0) | 2026-06-14 | Versión Menor | Colores de marca del editor: 5 nuevos campos de apariencia (`color_accent`, `color_accent_fg`, `color_canvas`, `color_muted`, `color_muted_fg`) en la configuración de empresa y espacio de trabajo; `color_palette` resuelto ahora devuelve 11 claves |
| [v1.24.0](#v1-24-0) | 2026-06-09 | Versión Menor | Claves API de prueba del espacio de trabajo: la creación devuelve un par live+test, las lecturas devuelven ambas, regeneración/expiración según `key_type` |
| [v1.23.0](#v1-23-0) | 2026-06-04 | Versión Menor | Endpoints de términos del firmante, endpoints de configuración de empresa, configuración de aceptación de términos |
| [v1.22.1](#v1-22-1) | 2026-06-02 | Versión de Parche | Ahora se puede eliminar el dominio de correo personalizado principal/único (DELETE ya no devuelve 400) |
| [v1.22.0](#v1-22-0) | 2026-05-30 | Versión Menor | Endpoint de eliminación de solicitud de firma, evento de webhook signing_request.deleted |
| [v1.21.1](#v1-21-1) | 2026-05-19 | Versión de Parche | Activación del webhook del espacio de trabajo vía API |
| [v1.21.0](#v1-21-0) | 2026-05-14 | Versión Menor | Configuración de la parte local del remitente de correo personalizado |
| [v1.20.0](#v1-20-0) | 2026-05-12 | Versión Menor | Configuración de control de navegación guiada |
| [v1.19.0](#v1-19-0) | 2026-05-12 | Versión Menor | Endpoints de gestión de logotipo, icon_url del espacio de trabajo |
| [v1.18.0](#v1-18-0) | 2026-05-06 | Versión Menor | Campos editables de identidad, datos precompletados editables, validación de valor de campo |
| [v1.17.0](#v1-17-0) | 2026-05-01 | Versión Menor | Personalización de color para la experiencia de firma |
| [v1.16.0](#v1-16-0) | 2026-04-27 | Versión Menor | Endpoint de Descarga de Solicitud de Firma |
| v1.15.3 | 2026-04-27 | Menor | Alias de campos limpios |
| [v01.15.02](#v01-15-02) | 2026-04-23 | Versión de Parche | Se agregaron tipos de campo faltantes al esquema de campos de create-and-send |
| [v01.15.01](#v01-15-01) | 2026-04-17 | Versión de Parche | Se agregó soporte de idioma ruso y polaco a los esquemas de la API |
| [v01.15.00](#v01-15-00) | 2026-04-16 | Versión Menor | Webhooks de espacio de trabajo, endpoints de rotación/estado de secretos |
| [v01.14.00](#v01-14-00) | 2026-04-14 | Versión Menor | Designaciones de Aprobador/CC, tipos de campo de aprobación |
| [v01.13.00](#v01-13-00) | 2026-04-12 | Versión Menor | Tipo de campo de sello |
| [v01.12.01](#v01-12-01) | 2026-04-02 | Versión de Parche | Nombres de variable de campo personalizado (`variable_defined_name`) |
| [v01.12.00](#v01-12-00) | 2026-03-26 | Versión Menor | Integración con servidor MCP |
| [v01.11.00](#v01-11-00) | 2026-03-19 | Versión Menor | Campos de subida de archivos, etiquetas de anclaje, colores de fondo de campo, cambio del valor predeterminado del orden de firma |
| [v01.10.00](#v01-10-00) | 2026-03-10 | Versión Menor | Campos condicionales, soporte DOCX, registro de auditoría, control del marco de firma |
| [v01.09.00](#v01-09-00) | 2026-03-05 | Versión Menor | Verificación OTP, reemplazo de documento de plantilla |
| [v01.08.00](#v01-08-00) | 2026-03-03 | Versión Menor | API de plantillas de correo, soporte de idiomas, observabilidad de webhooks |
| [v01.07.00](#v01-07-00) | 2026-02-25 | Versión Menor | Firmas manuscritas, campos de configuración heredados |
...
**Tipo de lanzamiento:** Versión menor

**Especificación OpenAPI:** [Descargar la especificación v1.38.0](/api-reference/v01.38.00/openapi-v01.38.00.es.json)

### Compatibilidad con catalán

El conjunto de idiomas admitidos crece de 14 a 15 con la incorporación del catalán (`ca`), que se suma a `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro` y `nb`.

El nuevo código se acepta en todas las superficies del API que llevan idioma:

- **Configuración del espacio de trabajo**: `language` en `PUT /workspace/{workspace_id}/settings` (y devuelto por el GET correspondiente)
- **Configuración de la empresa**: `language` en los endpoints de configuración de empresa
- **Idioma de correo por solicitud de firma**: el campo `language` en `POST /signing-requests`, `POST /signing-requests/create-and-send` y `PATCH` / `PUT /signing-requests/{id}`
- **Términos del firmante**: el parámetro de ruta `{language}` en los endpoints de términos del firmante de empresa y espacio de trabajo (`PUT` / `DELETE`)
- **Plantillas de correo predeterminadas**: `GET /email-templates/defaults/{language}`
- **Declaración del sello de organización**: `statement.language` en la creación y actualización del sello, devuelto como `statement_language` en el sello

Establecer el catalán localiza la experiencia completa del firmante: correos dirigidos al firmante, la interfaz de la ceremonia de firma, la declaración de consentimiento registrada al aceptar los términos y el formato de fechas. El certificado de finalización sigue el idioma del espacio de trabajo, así que establezca el catalán en el espacio de trabajo para obtener un certificado en catalán.

Los errores de validación para códigos no admitidos ahora enumeran los 15 idiomas.

#### Notas de migración

- El cambio del API es aditivo. Los 14 códigos de idioma existentes se aceptan y se devuelven exactamente igual que antes, y las solicitudes que nunca envían `ca` se validan como antes.
- Salvo que la URL de firma incluya un código `lang` admitido, la página de firma selecciona el primer idioma de la lista de preferencias del navegador del firmante que admite Firma.dev. Un firmante cuyo navegador incluye `ca` o una variante regional `ca-` antes que cualquier otro idioma admitido ahora ve la página de firma en catalán. Antes de v1.38.0, el firmante veía el siguiente idioma admitido de la lista o el inglés. Los enlaces de firma enviados por correo electrónico no incluyen un parámetro `lang`, por lo que este comportamiento también se aplica a los firmantes existentes. Un código `lang` admitido que añadas a una URL de firma incrustada sigue estableciendo el idioma; se ignora cualquier otro valor.
- La declaración de consentimiento se resuelve en este orden: términos del espacio de trabajo en el idioma del firmante, términos de la empresa en el idioma del firmante, términos del espacio de trabajo en el idioma configurado del espacio de trabajo, términos de la empresa en el idioma configurado de la empresa y, por último, los términos predeterminados de Firma.dev en el idioma del firmante. Si no existen términos en catalán ni en el idioma configurado aplicable, un firmante que prioriza el catalán ahora acepta la declaración predeterminada en catalán de Firma.dev en lugar de términos personalizados en otro idioma del navegador, como el español. Los términos en el idioma configurado del espacio de trabajo o de la empresa siguen aplicándose cuando están disponibles.
- Añade términos del firmante en catalán a nivel de empresa o de espacio de trabajo para conservar los términos personalizados para estos firmantes. Usa `PUT /company/signer-terms/ca`, `PUT /workspace/{workspace_id}/signer-terms/ca` o la configuración de términos del panel de control.
...
**Tipo de lanzamiento:** Versión de parche

**Especificación OpenAPI:** [Descargar la especificación v1.37.0](/api-reference/v01.37.00/openapi-v01.37.00.es.json) (sin cambios en esta versión)

### Destinatarios CC

Los endpoints de actualización ahora guardan los destinatarios con `"designation": "CC"` y los endpoints de lectura ahora los devuelven. Antes de esta versión no hacían ninguna de las dos cosas.

#### Añadir y actualizar destinatarios CC

- `PUT /templates/{id}` (`users[]`) y `PATCH /templates/{id}` (`user`) guardan destinatarios CC. Antes devolvían `400` y `500`.
- `PUT /signing-requests/{id}` (`recipients[]`) y `PATCH /signing-requests/{id}` (`recipient`) guardan destinatarios CC. Antes, `PUT` devolvía `200` y contaba el CC en `summary.recipients_created` sin guardarlo, y `PATCH` devolvía `500`.
- En `PUT /signing-requests/{id}`, un destinatario que no se puede guardar ahora aparece en `details.recipients` de la respuesta `400` en lugar de contarse en `summary.recipients_created`.
- Los destinatarios CC existentes se pueden actualizar por `id` y eliminar mediante `deleted_users` / `deleted_recipients`, igual que los firmantes y los aprobadores.
- `first_name` y `email` son obligatorios para un nuevo destinatario CC.
- `designation` se compara sin distinguir mayúsculas de minúsculas (`"CC"`, `"cc"`).

#### Destinatarios CC en las respuestas

Los destinatarios CC se devuelven con `"designation": "CC"`, después de los firmantes y los aprobadores, en:

- `GET /signing-requests/{id}/users`
- `GET /templates/{id}/users`
- `GET /templates/{id}` y `GET /templates` (`recipients`)
- el array `recipients` de las respuestas de `POST /signing-requests`, `POST /signing-requests/create-and-send` y `PUT /signing-requests/{id}`

Las entradas CC tienen las mismas claves que las demás entradas de ese endpoint. Nunca reciben un enlace de firma, y las claves que solo se aplican a la firma tienen valores neutros (por ejemplo `required_fields: []` y `ready_to_send: true`).

#### Creación a partir de una plantilla

- `POST /signing-requests` y `POST /signing-requests/create-and-send` con `template_id` ahora copian los destinatarios CC de la plantilla a la nueva solicitud de firma. Un CC de la plantilla se omite cuando la solicitud ya tiene un CC con el mismo email (sin distinguir mayúsculas de minúsculas).
- Un destinatario del cuerpo de la solicitud con `"designation": "CC"` siempre se añade como destinatario CC. Nunca se asocia a un usuario de la plantilla por `template_user_id` ni por `order`.

#### Validación

- Cambiar un destinatario existente entre CC y Signer o Approver devuelve `400`. `PATCH /templates/{id}` y `PATCH /signing-requests/{id}` devuelven el mensaje `Cannot change designation between CC and Signer/Approver; delete the recipient and create it again`, `PUT /signing-requests/{id}` lo incluye en `details.recipients` y `PUT /templates/{id}` devuelve el mensaje genérico `Validation errors occurred`. Elimina el destinatario y vuelve a crearlo con la nueva designación.
- No se puede asignar un campo a un destinatario CC. En `PUT /signing-requests/{id}`, incluido un CC creado en la misma solicitud mediante `_temp_id`, el error del campo dice `Cannot assign fields to CC recipient.`; las demás rutas de campos rechazan el id de un CC como destinatario no válido.
- Actualizar o eliminar un destinatario por `id` ahora solo encuentra destinatarios que no se han eliminado. Un destinatario ya eliminado se trata como no encontrado en lugar de modificarse.

#### Notas de migración

- Sin nuevos endpoints, campos ni valores de enum, por lo que la especificación v1.37.0 sigue siendo válida.
- Las listas de destinatarios ahora pueden incluir entradas CC. Si tu código trata cada entrada como un firmante, comprueba `designation`.
- Las solicitudes de firma creadas a partir de una plantilla con destinatarios CC ahora incluyen esos destinatarios CC, que reciben el documento completado. Elimina los destinatarios CC de la plantilla si no quieres este comportamiento.
- Si dependías de que `PUT /signing-requests/{id}` devolviera `200` cuando un destinatario no se guardaba, gestiona la respuesta `400`.
...
**Especificación OpenAPI:** [Descargar la especificación v1.37.0](/api-reference/v01.37.00/openapi-v01.37.00.es.json)

### Sellos de organización

Sellos electrónicos aplicados automáticamente en nombre de una empresa, como participantes en el orden de firma. La configuración, las reglas de ámbito y la salida en el certificado se describen en la [guía de Sellos de Organización](/guides/organization-seals).

#### Nuevos endpoints

| Método | Ruta | Propósito |
|---|---|---|
| `POST` | `/seals` | Crear un sello (`kind`: `uploaded`, `typed` o `drawn`; `scope`: `company` o `workspace` con `workspace_id`). Requiere clave API protegida para el ámbito de empresa |
| `GET` | `/seals` | Listar los sellos visibles en el ámbito de la clave |
| `GET` | `/seals/{id}` | Obtener un sello |
| `PATCH` | `/seals/{id}` | Reemplazar la imagen (nueva versión), renombrar, actualizar `display_name` / `signatory_title` o establecer `is_default` |
| `DELETE` | `/seals/{id}` | Revocar; añade `?stop_pending=true` para pausar los participantes en tránsito |
| `GET` | `/seals/{id}/image` | PNG canónico; cada lectura se registra |
| `GET` | `/seals/{id}/applications` | Solicitudes de firma en las que se aplicó el sello |
| `GET` | `/seals/{id}/access-log` | Eventos de ciclo de vida y de lectura de imagen |
| `DELETE` | `/seals/{id}/erase` | Borrar un linaje revocado no referenciado por ninguna solicitud ni plantilla, 90 días después de la revocación |
| `DELETE` | `/signing-requests/{id}/seal-participants/{participant_id}` | Eliminar un participante de sello y sus campos de una solicitud no enviada |

#### Participantes de sello

Las solicitudes de firma y las plantillas aceptan un array `seal_participants[]` junto a `recipients[]` / `users[]` en `POST /signing-requests`, `POST /signing-requests/create-and-send`, `PATCH /signing-requests/{id}`, `POST /templates` y `PATCH /templates/{id}`. Cada entrada incluye `temp_id` (identificador del cliente para asignar campos), `seal_id` y `order`. Destinatarios y sellos comparten un único espacio de orden contiguo; el orden 1 se aplica al enviar.

Los campos que pertenecen a un sello indican `seal_participant_temp_id` en lugar de un destinatario. Los campos de sello los genera el servidor (`read_only: 1`, sin destinatario) y los valores proporcionados por el cliente en ellos se ignoran. Las respuestas de solicitudes de firma incluyen ahora `seal_participants[]`.

#### Intercambio de sello

`PATCH /signing-requests/{id}` acepta `seal_participant: { "id": "<participant-id>", "swap_to_seal_id": "<seal-uuid>" }` para mover un participante pausado de una solicitud enviada a otro sello del ámbito. Requiere clave API protegida.

#### Eventos de webhook

| Evento | Disparador |
|---|---|
| `organization_seal.created` | Sello creado |
| `organization_seal.updated` | Imagen reemplazada (nueva versión), renombrado o marca de predeterminado cambiada |
| `organization_seal.deleted` | Sello revocado |
| `organization_seal.erased` | Linaje revocado borrado permanentemente |
| `signing_request.seal.applied` | Sello aplicado a un documento |
| `signing_request.seal.paused` | Participante de sello pausado (revocado con detención de pendientes, o discrepancia de integridad) |
| `signing_request.seal.swapped` | Participante pausado intercambiado por otro sello |

#### Códigos de error

`SEALS_DISABLED`, `SEAL_NOT_FOUND`, `SEAL_SCOPE_FORBIDDEN`, `SEAL_IMAGE_INVALID`, `SEAL_ALREADY_REVOKED`, `SEAL_ERASE_NOT_ELIGIBLE`, `SEAL_MUTATION_NOT_ALLOWED`, `SEAL_ORDER_COLLISION`, `SEAL_PAUSED`, `SEAL_UNAVAILABLE` y `SEAL_CREATION_DISABLED_EDGE` (reservado para el host edge de respaldo; el host principal de la API realiza la creación de sellos).

#### Notas de migración

- Puramente aditivo. `recipients[]`, `users[]` y todos los endpoints existentes no cambian.
...
**Especificación OpenAPI:** [Descargar la especificación v1.36.0](/api-reference/v01.36.00/openapi-v01.36.00.es.json)

### Anchor tags

- `add_white_background` en las definiciones de anchor tags: pinta un fondo blanco detrás del campo anclado.

### Respuestas de error

- Los objetos de error incluyen un `code` legible por máquina.
- Los fallos de validación devuelven además `errors[]` con los errores por campo.

### Create and send

- Las respuestas de `POST /signing-requests/create-and-send` incluyen `warnings[]` con avisos no bloqueantes.

#### Notas de migración

- Puramente aditivo. Sin cambios incompatibles.
...
**Especificación OpenAPI:** [Descargar spec v1.35.0](/api-reference/v01.35.00/openapi-v01.35.00.es.json)

### Copia de plantilla entre espacios de trabajo

Nuevo endpoint `POST /templates/{id}/copy` que copia una plantilla a otro espacio de trabajo dentro de la misma empresa. Todos los datos asociados se copian con remapeo correcto de IDs:

- Configuraciones de la plantilla (vencimiento, orden de firma, correos, etc.)
- Destinatarios (firmantes, aprobadores) con designaciones preservadas
- Campos con referencias de `required_conditions` y `visibility_conditions` remapeadas
- Destinatarios en copia (CC)
- Definiciones de campos personalizados
- Recordatorios con referencias a destinatarios remapeadas
- Documento PDF (copia independiente)

**Cuerpo de la solicitud:**

- `workspace_id` (requerido, uuid) - espacio de trabajo destino
- `name` (opcional, string, max 255) - nombre para la copia (por defecto, nombre original con sufijo " (copy)")

**Autorización:** requiere una clave API de nivel empresa (protegida). Las claves de espacio de trabajo reciben `403 PROTECTED_KEY_REQUIRED`.
...
[Descargar especificación OpenAPI](/api-reference/v01.33.00/openapi-v01.33.00.json)

### Cinco nuevos idiomas

El conjunto de idiomas admitidos crece de 9 a 14: checo (`cs`), sueco (`sv`), neerlandés (`nl`), rumano (`ro`) y noruego bokmål (`nb`) se suman a `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`.

Los nuevos códigos se aceptan en todas las superficies del API que llevan idioma:

- **Configuración del espacio de trabajo** — `language` en `PUT /workspace/{workspace_id}/settings` (y devuelto por el GET correspondiente)
- **Configuración de la empresa** — `language` en los endpoints de configuración de empresa
- **Idioma de correo por solicitud de firma** — el campo `language` en `POST /signing-requests`, `POST /signing-requests/create-and-send` y `PATCH` / `PUT /signing-requests/{id}`
- **Términos del firmante** — el parámetro de ruta `{language}` en los endpoints de términos del firmante de empresa y espacio de trabajo (`GET` / `PUT` / `DELETE`)
- **Plantillas de correo predeterminadas** — `GET /email-templates/defaults/{language}`

Establecer uno de los nuevos idiomas localiza la experiencia completa del firmante: correos dirigidos al firmante, la interfaz de la ceremonia de firma, la declaración de consentimiento registrada al aceptar los términos, el formato de fechas y el certificado de finalización (incluida la representación correcta de los diacríticos checos y rumanos).

Los errores de validación para códigos no admitidos ahora enumeran los 14 idiomas.

### Guía de migración

No se requiere ninguna acción. El cambio es puramente aditivo: los 9 códigos de idioma existentes se comportan exactamente igual que antes, y los espacios de trabajo o solicitudes de firma que nunca establecieron un idioma no se ven afectados.
