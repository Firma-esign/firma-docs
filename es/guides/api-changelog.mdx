...
Nuevo campo opcional `language` en `POST /signing-requests/create-and-send` y `PATCH /signing-requests/{id}`. Establécelo en uno de `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl` y todos los correos dirigidos al firmante de esa solicitud de firma — invitación, recordatorios, verificación OTP, finalización y CC, expiración, cancelación y rechazo — junto con su formato de fecha, usan ese idioma.
