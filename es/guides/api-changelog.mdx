...
- Cuando la URL de firma no tiene un parámetro `lang`, la página de firma selecciona el primer idioma de la lista de preferencias del navegador del firmante que admite Firma.dev. Un firmante cuyo navegador incluye `ca` o una variante regional `ca-` antes que cualquier otro idioma admitido ahora ve la página de firma en catalán. Antes de v1.38.0, el firmante veía el siguiente idioma admitido de la lista o el inglés. Los enlaces de firma enviados por correo electrónico no incluyen un parámetro `lang`, por lo que este comportamiento también se aplica a los firmantes existentes. Un parámetro `lang` que añadas a una URL de firma incrustada sigue estableciendo el idioma.
