...
<Steps>
  ...
  <Step title="Finaliza para obtener los registros de envío">
    ...
    <Warning>
      Si la finalización del dominio falla porque el dominio ya está registrado en otra cuenta de Resend, consulta [¿Ya usas Resend para tu propio correo?](#already-using-resend-for-your-own-email) más abajo. El trabajo en segundo plano de finalización automática (se ejecuta en un plazo de ~5 minutos) establece `status_message: "resend_conflict"` en el registro del dominio - verifica el estado del dominio mediante `GET /company/domains` para detectarlo.
    </Warning>
  </Step>
...
Firma envía a través de su propia cuenta de Resend por debajo. Resend no permite que el mismo dominio se registre bajo dos cuentas distintas a la vez. Cuando el trabajo en segundo plano de finalización automática de Firma detecta un conflicto, establece `status_message: "resend_conflict"` en el registro del dominio. Esto aparece en el panel como una insignia de **Conflicto de Dominio** en el dominio.
...
| --- | --- | --- | --- |
| Pending Ownership | `0` | `0` | Esperando el registro TXT `_firma-verification` y una llamada a `verify-ownership` |
| Configuring | `1` | `0` | Propiedad confirmada, esperando `finalize` para registrarse con el proveedor de correo y emitir los registros SPF/DKIM/DMARC |
| Not Verified | `2` | `0` | Registros emitidos, esperando la propagación de DNS y una llamada exitosa a `verify-dns` |
...
| Domain Conflict | cualquiera | cualquiera | Consulta [¿Ya usas Resend para tu propio correo?](#already-using-resend-for-your-own-email) |

### Bloqueado en "Configuring"

Si un dominio permanece en `verification_status = 1` durante más de unos minutos sin avanzar, el trabajo en segundo plano de Firma reintenta automáticamente la llamada a `finalize`. Si sigue bloqueado después de eso, la causa subyacente casi siempre es el conflicto con Resend descrito arriba - verifica si el `status_message` del dominio es `resend_conflict` mediante `GET /company/domains` antes de contactar con soporte.
...
- [Webhooks](/guides/webhooks) - suscríbete a los eventos `domain.verified` y `domain.verification.failed` para rastrear la verificación de propiedad (Paso 2); no hay webhook para el paso de verificación de DNS - sondea `verify-dns` para eso
