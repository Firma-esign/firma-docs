...
| :--- | :--- | :--- | :--- |
| `variable_name` | Propiedad de campo de nivel superior (solicitud y respuesta) | Una etiqueta que asignas a un campo. Se usa como coincidencia de respaldo al fusionar campos en una solicitud de firma creada a partir de una plantilla. **Nunca lee ni escribe datos del destinatario.** | No |
| `read_only_value` | Propiedad de campo de nivel superior (solicitud) | Texto estático que proporcionas al momento de crear el campo. **No se puede combinar con `format_rules.prefilledData` en el mismo campo** — la API devuelve 400 si ambos están configurados. Cuando una plantilla ya tiene una regla `prefilledData`, `read_only_value` en el campo por solicitud sobrescribe la regla de la plantilla durante la fusión. | Sí — siempre gana cuando está presente |
| `format_rules.prefilledData` | Anidada dentro de `format_rules` en el campo | La clave de atributo del destinatario desde la cual autocompletar (`email`, `full_name`, `phone_number`, `company`, etc., o una clave `custom_fields` del destinatario). Se resuelve en el momento del envío a partir de los datos del destinatario asignado. **No se puede combinar con `read_only_value` en el mismo campo** — la API devuelve 400. | Sí — cuando `read_only_value` está ausente |
