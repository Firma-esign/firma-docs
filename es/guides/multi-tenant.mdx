...
Como el JWT se genera para una plantilla específica (y esa plantilla pertenece a un espacio de trabajo específico), el límite entre clientes lo impone el propio token - un token generado para la plantilla del Cliente A no se puede reutilizar contra los datos del Cliente B. Los editores integrados heredan la paleta de colores del espacio de trabajo una vez configurada - si no se han establecido colores personalizados, recurren a la paleta predeterminada de Firma. Establece los campos `color_*` en la configuración del espacio de trabajo para aplicar la marca de tu cliente. Un token correctamente delimitado te da aislamiento entre clientes; agregar colores personalizados completa la marca blanca.
