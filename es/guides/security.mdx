...
**Respuesta rápida:** La infraestructura de Firma se ejecuta íntegramente en la UE, con cifrado en tránsito y en reposo, control de acceso a nivel de fila en cada tabla de la base de datos, y un proceso de firma respaldado por un registro de auditoría completo. Firma **no** cuenta actualmente con certificación SOC 2 ni ISO 27001, y no gestiona un programa formal de recompensas por vulnerabilidades (bug bounty). Los reportes de seguridad son bienvenidos y se atienden. Más detalles a continuación.
...
| Sellado de firma digital | DSS (Digital Signature Service) | Aplica sellos criptográficos PAdES a los documentos completados (habilitado por espacio de trabajo — aún no se aplica a todos los documentos por defecto) |
...
Firma es directo sobre su situación actual:

- **Sin certificación SOC 2.** Firma no cuenta actualmente con un informe SOC 2 Tipo I o Tipo II.
- **Sin certificación ISO 27001.** Firma no está actualmente certificada bajo ISO 27001.
...
Si tu proceso de adquisición exige alguno de estos requisitos como condición obligatoria, Firma aún no cumple ese estándar — preferimos decírtelo directamente antes de que surja más tarde en un cuestionario de seguridad. Esta página se actualizará si ese estado cambia.

<Note>
  Para el aspecto de validez legal del cumplimiento normativo — niveles de firma eIDAS, ESIGN/UETA y normativas específicas por país — consulta [Validez Legal y Cumplimiento de eIDAS](/guides/legal-validity). Esa página también cubre la ausencia actual de certificación SOC 2/ISO 27001 en el contexto del peso legal de la firma.
</Note>
...
- [Validez Legal y Cumplimiento de eIDAS](/guides/legal-validity) — niveles de firma, admisibilidad y la misma divulgación sobre SOC 2/ISO 27001 en un contexto legal
