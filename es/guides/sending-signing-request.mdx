...
| | `POST /signing-requests` (crear) | `POST /signing-requests/create-and-send` |
| :--- | :--- | :--- |
| Resultado | Un **borrador** (`status: "draft"`). No se envía nada. | La solicitud se crea **y** se envía, de forma atómica, en una sola llamada. |
| ¿Se notifica a los destinatarios? | Nunca - sin importar `send_signing_email` ni ninguna otra configuración. | Por defecto, sí. Consulta `send_signing_email` más abajo. |
| ¿Se cobra crédito / se establece `sent_on`? | No | Sí, de inmediato. |
| ¿Sigue siendo editable mediante PATCH/PUT? | Sí, hasta que llames a `/send` | No - la solicitud ya está en estado enviado. |
| Ideal para | Un flujo de **borrador y luego edición**: construir la solicitud, ajustar destinatarios o campos, que se revise internamente y luego enviarla cuando esté lista. | Una llamada de **enviar y olvidar** cuando ya tienes todo lo que necesitas. |

Usa [`create`](/api-reference/v01.33.00/signing-requests/create-signing-request) cuando tu flujo de trabajo necesite un paso de revisión o edición antes de que algo llegue a un destinatario. Usa [`create-and-send`](/api-reference/v01.33.00/signing-requests/create-and-send-signing-request-atomic) cuando quieras que la solicitud quede activa y los destinatarios notificados en una sola llamada.
...
| Endpoint | Efecto de `send_signing_email: false` |
| :--- | :--- |
| `POST /signing-requests` (crear) | Ninguno. Este endpoint nunca envía un correo por sí solo, así que el indicador simplemente se almacena para más tarde - solo tiene efecto una vez que el borrador se envía realmente. |
| `POST /signing-requests/create-and-send` | La solicitud igual se crea **y** se envía: se cobra el crédito, se establece `sent_on`, se dispara el webhook `signing_request.sent`, y el enlace de firma queda activo. Solo se suprime el **correo electrónico** de notificación al destinatario. |
...
<Tip>
  ¿Quieres que un campo muestre un valor antes de que el firmante lo toque - un valor fijo que ya conoces, o los propios datos de perfil del destinatario? Consulta la [guía de Prellenado de campos](/guides/field-prefilling). `variable_name` por sí solo **no** hace esto.
</Tip>
...
Renderiza esta URL en un iframe. Para conocer la configuración completa de incrustación - incluidos los eventos postMessage, el dimensionamiento del iframe y los permisos de cámara/portapapeles - consulta la [guía de Firma incrustable](/guides/embeddable-signing).
...
- [Prellena campos con datos del destinatario o datos estáticos](/guides/field-prefilling) antes de enviar
