...
| --- | --- | --- |
| `enabled` por webhook | Una suscripción de webhook individual | Se configura en el propio webhook (`PATCH /webhooks/{id}`) |
...
<Note>
  **Límite de tasa**: Las operaciones de lectura (GET) de webhooks admiten **200 solicitudes por minuto por clave API**. Las operaciones de escritura de webhooks (crear, actualizar, eliminar) están limitadas a **60 solicitudes por minuto**.
</Note>
