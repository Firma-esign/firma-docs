...
### Eventos de sello de organización

- `organization_seal.created` - Nuevo sello creado
- `organization_seal.updated` - Imagen del sello reemplazada (nueva versión), sello renombrado o marca de predeterminado cambiada
- `organization_seal.deleted` - Sello revocado
- `organization_seal.erased` - Linaje de sello revocado borrado permanentemente

### Eventos de sello en solicitudes de firma

- `signing_request.seal.applied` - Sello aplicado a un documento
- `signing_request.seal.paused` - Participante de sello pausado (sello revocado con detención de pendientes, o discrepancia de integridad)
- `signing_request.seal.swapped` - Participante de sello pausado intercambiado por otro sello
...
La entrega de webhooks requiere que **ambos interruptores estén activados**: un interruptor por webhook y un interruptor maestro a nivel de cuenta. Los dos son independientes, por lo que un webhook puede crearse y habilitarse correctamente sin llegar a disparar un solo evento.

| Interruptor | Alcance | Dónde se configura |
| --- | --- | --- |
| `enabled` por webhook | Una suscripción de webhook individual | Se configura en el propio webhook (`PUT /webhooks/{id}`) |
| Interruptor maestro | Empresa o espacio de trabajo | Empresa: únicamente el interruptor del panel en **Configuración > Webhooks > Habilitar Webhooks**. Espacio de trabajo: desde el panel, o mediante `PATCH /workspaces/{id}` con `{"webhook_enabled": true}` |

Al menos un alcance (empresa o espacio de trabajo) debe tener su interruptor maestro activado para que se entregue cualquier webhook de ese alcance. Activar el interruptor maestro del espacio de trabajo también genera un secret de firma para ese espacio de trabajo si aún no existe uno.
...
3. Después de 7 días, solo se envía `X-Firma-Signature`
...
**Los webhooks están configurados pero los eventos no se disparan**

Esta es la causa más común de los reportes de "mi webhook no funciona". La configuración por webhook puede ser completamente correcta mientras el interruptor maestro a nivel de cuenta está desactivado, lo que omite silenciosamente todos los eventos.

- Confirma que el [interruptor maestro](#two-levels-of-enablement) está activado para el alcance correcto: los webhooks a nivel de empresa necesitan el interruptor de la empresa activado, los webhooks a nivel de espacio de trabajo necesitan el interruptor de ese espacio de trabajo activado.
