...
<Note>
  Todos los ejemplos de la API de esta página pasan tu clave API directamente en el encabezado `Authorization`. El prefijo `Bearer` es opcional - tanto `Authorization: YOUR_API_KEY` como `Authorization: Bearer YOUR_API_KEY` funcionan.
</Note>
...
| `color_accent` | Color de acento secundario | `#34eeff` |
| `color_accent_fg` | Color de texto en elementos de acento | `#000000` |
| `color_canvas` | Fondo del lienzo/visor de documentos | `#0f1419` |
| `color_muted` | Áreas de fondo atenuadas/sutiles | `#22222a` |
| `color_muted_fg` | Color de texto en áreas atenuadas | `#b8b8b8` |
...
| `show_qr_code` | Incluye un código QR que enlaza a la página de firma en los correos | `true` |
...
`require_terms_acceptance` sigue la misma cascada que otras configuraciones de marca: **configuración del espacio de trabajo → configuración de la empresa → valor predeterminado de Firma**. El valor predeterminado de Firma es `true` (habilitado), por lo que la aceptación de términos está ACTIVADA para todos los documentos a menos que la desactives explícitamente a nivel de empresa o de espacio de trabajo.
...
3. **Términos del espacio de trabajo** para el idioma configurado del espacio de trabajo
4. **Términos de la empresa** para el idioma configurado de la empresa
5. **Términos predeterminados de Firma** (mínima prioridad)

<Note>
  La aceptación de términos se rastrea por firmante, por solicitud de firma - no por dirección de correo electrónico del firmante. Un firmante que acepta los términos en un documento debe volver a aceptarlos en el siguiente documento; no existe un estado de "aceptar una vez" entre documentos.
</Note>
...
`require_terms_acceptance` se puede desactivar para toda tu cuenta o para un único espacio de trabajo.

**Desactivar para toda tu cuenta** (todos los espacios de trabajo, a menos que un espacio de trabajo lo sobrescriba):
...
**Desactivar para un único espacio de trabajo** (sobrescribe la configuración de la empresa solo para ese espacio de trabajo):
...
Establece `require_terms_acceptance` en `null` a nivel de espacio de trabajo para eliminar la anulación y volver a heredar la configuración de la empresa.

<Note>
  Si el interruptor del panel de un espacio de trabajo para esta configuración muestra un error de permisos, solo los administradores de la empresa o del espacio de trabajo pueden cambiar esta configuración desde la interfaz del panel. Los miembros del equipo sin acceso de administrador deben pedirle a un administrador que la actualice, o usar la API directamente con tu clave API, que no está sujeta a la restricción de administrador del panel.
</Note>

Idiomas admitidos: `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro`, `nb`, `ca`.
...
**Validación:**
- Cada etiqueta debe ser una cadena de texto: los valores de más de 200 caracteres se truncan automáticamente sin aviso
- Las claves de idioma son cadenas de texto (máximo 10 caracteres, no se validan contra la lista de idiomas admitidos)
...
```json
{
  ...
  "verification_instructions": {
    ...
    "record_value": "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
...
| Tipo | Nombre                           | Valor                    |
| ---- | ------------------------------ | ------------------------ |
| TXT  | `_firma-verification.acme.com` | `a1b2c3d4-e5f6-7890-abcd-ef1234567890` |
...
Consulta la [Referencia de la API de dominios de correo](/api-reference/v01.27.00/email-domains/list-company-domains) para la documentación completa de los endpoints, o [Dominios personalizados](/guides/custom-domains) para conocer los detalles de los registros DNS, la resolución de conflictos con Resend y la solución de problemas de estado de verificación.
...
**El nombre del remitente** es el nombre del espacio de trabajo (o el nombre de la empresa si el espacio de trabajo no tiene nombre definido).

Por ejemplo, si tu espacio de trabajo se llama "Acme Legal" y defines `email_local_part` como `noreply` con un dominio verificado `sign.acmecorp.com`, los correos se enviarán desde:

```
Acme Legal <noreply@sign.acmecorp.com>
```
...
Permite a los usuarios crear y editar plantillas dentro de tu aplicación usando autenticación JWT. El editor se carga mediante una etiqueta script y se monta en un elemento contenedor a través de Shadow DOM, no de un iframe:

```html
<div id="firma-template-editor"></div>
<script src="https://api.firma.dev/functions/v1/embed-proxy/template-editor.js"></script>
<script>
  const editor = new FirmaTemplateEditor({
    container: document.getElementById('firma-template-editor'),
    jwt: 'YOUR_JWT_TOKEN',
...
[Guía del editor de plantillas incrustado](embeddable-template-editor) - Generación de JWT, eventos e implementación completa

### Editor de solicitudes de firma incrustado

Mismo patrón: etiqueta script + constructor, no un iframe:

```html
<div id="firma-sr-editor"></div>
...
<script>
  const editor = new FirmaSigningRequestEditor({
    container: document.getElementById('firma-sr-editor'),
    jwt: 'YOUR_JWT_TOKEN',
    signingRequestId: 'YOUR_SIGNING_REQUEST_ID'
  })
</script>
```
...
- El tiempo de expiración del token es fijo en el servidor: 24 horas para los tokens del editor de plantillas, 7 días para los tokens de solicitudes de firma - no configurable por quien realiza la llamada
