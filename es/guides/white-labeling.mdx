...
<Note>
  Si un interruptor del panel de un espacio de trabajo para esta configuración muestra un mensaje de éxito pero el valor en realidad no cambia, esto es un problema de permisos, no un error: solo los administradores de la empresa o del espacio de trabajo pueden cambiar esta configuración desde la interfaz del panel. Los miembros del equipo sin acceso de administrador deben pedirle a un administrador que la actualice, o usar la API directamente con tu clave API, que no está sujeta a la restricción de administrador del panel.
</Note>
