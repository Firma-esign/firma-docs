...
| Version | Date | Type de version | Changements clés |
|---------|------|--------------|-------------|
| [v1.38.0](#v1-38-0) | 2026-09-17 | Version mineure | Prise en charge du catalan : l'ensemble des langues prises en charge par l'API passe à 15, accepté partout où un code de langue est accepté, y compris les paramètres d'espace de travail et d'entreprise, la langue d'e-mail par demande de signature, les conditions du signataire, les modèles d'e-mail par défaut et la langue de la déclaration du cachet d'organisation |
| [v1.37.1](#v1-37-1) | 2026-09-15 | Version corrective | Destinataires CC : acceptés sur `PUT`/`PATCH` des modèles et des demandes de signature, renvoyés dans toutes les listes de destinataires et copiés depuis le modèle lors de la création d'une demande de signature à partir d'un modèle |
| [v1.37.0](#v1-37-0) | 2026-09-14 | Version mineure | Cachets d'organisation : endpoints du cycle de vie du cachet sous `/seals`, `seal_participants[]` sur les modèles et demandes de signature, échange de cachet sur les demandes envoyées, sept nouveaux événements webhook |
| [v1.36.0](#v1-36-0) | 2026-08-31 | Version mineure | Anchor tags : `add_white_background` ; réponses d'erreur structurées (`code`, `errors[]`) ; `warnings[]` dans les réponses create-and-send |
| [v1.35.0](#v1-35-0) | 2026-08-13 | Version Mineure | Copie de modele entre espaces de travail : nouveau endpoint `POST /templates/{id}/copy` pour copier un modele vers un autre espace de travail (cle API protegee requise) |
| [v1.33.0](#v1-33-0) | 2026-08-03 | Version Mineure | Cinq nouvelles langues — tchèque (`cs`), suédois (`sv`), néerlandais (`nl`), roumain (`ro`) et norvégien bokmål (`nb`) — acceptées partout où un code de langue est accepté : paramètres d'espace de travail/entreprise, langue d'e-mail par demande de signature, conditions du signataire et modèles d'e-mail par défaut |
| [v1.32.0](#v1-32-0) | 2026-07-22 | Version Mineure | Taille de police explicite par champ : `fontSize` optionnel (8-48 px) dans `format_rules` pour les champs textuels, appliqué dans la vue de signature et le PDF scellé avec réduction automatique préservée ; nouvelle propriété `font_size` sur les anchor tags |
| [v1.31.0](#v1-31-0) | 2026-07-14 | Version Mineure | Langue des e-mails optionnelle par demande de signature : le nouveau champ `language` sur create-and-send et update définit la langue de tous les e-mails destinés au signataire (revient à la valeur par défaut de l'espace de travail/entreprise si omis) |
| [v1.30.0](#v1-30-0) | 2026-07-08 | Version Mineure | Récupération des marques du signataire : images de signature / initiales / tampon (PNG en base64) et téléchargement des fichiers téléversés par le signataire, par signataire, via l'API |
| [v1.29.0](#v1-29-0) | 2026-07-06 | Version Mineure | Téléversement de document en deux étapes (`POST /documents`) pour les fichiers volumineux jusqu'à 50 Mo ; nouveau champ `document_id` à la création et à la création-et-envoi |
| [v1.28.0](#v1-28-0) | 2026-07-05 | Version Mineure | Exemples de code SDK TypeScript (`@firma-dev/sdk`) ajoutés à chaque endpoint ; endpoints de demande de signature intégrée et endpoint legacy `/documents` retirés de la référence |
| [v1.27.0](#v1-27-0) | 2026-06-29 | Version Corrective | Schémas de paramètres documentés : les réponses de demande de signature exposent désormais `show_qr_code`, `allow_presigning_download`, `identity_editable_fields`, `notify_identity_change_email` ; les réponses de modèle exposent `show_qr_code` ; `cancel` renvoie `emails_sent` ; `resend` renvoie `recipients_failed` ; nouveaux schémas `TemplateSettings` et `LegacyDocumentSettings` |
| [v1.26.0](#v1-26-0) | 2026-06-23 | Version Mineure | Paramètres de l'espace de travail : personnalisation des libellés du bouton de signature, bascule du code QR, bascule du téléchargement avant signature, bascule du filigrane partiel |
| [v1.25.2](#v1-25-2) | 2026-06-18 | Version Corrective | Les champs fournis qui ne correspondent à aucun champ de modèle renvoient désormais `400` (auparavant ignorés silencieusement) sur `POST /signing-requests` et `/create-and-send` ; les valeurs de `type` de champ sont désormais comparées sans distinction de casse |
| [v1.25.1](#v1-25-1) | 2026-06-14 | Version Corrective | Les échecs de l'étape d'envoi de `create-and-send` renvoient désormais le statut réel (`400`/`402`/`422`) avec un `code` exploitable par machine, au lieu de les masquer en `500` |
| [v1.25.0](#v1-25-0) | 2026-06-14 | Version Mineure | Couleurs de marque de l'éditeur : 5 nouveaux champs d'apparence (`color_accent`, `color_accent_fg`, `color_canvas`, `color_muted`, `color_muted_fg`) sur les paramètres d'entreprise et d'espace de travail ; le `color_palette` résolu renvoie désormais 11 clés |
| [v1.24.0](#v1-24-0) | 2026-06-09 | Version Mineure | Clés API de test pour l'espace de travail : la création renvoie une paire clé live + clé de test, les lectures renvoient les deux, régénération/expiration tenant compte de `key_type` |
| [v1.23.0](#v1-23-0) | 2026-06-04 | Version Mineure | Endpoints des conditions du signataire, endpoints des paramètres d'entreprise, paramètres d'acceptation des conditions |
| [v1.22.1](#v1-22-1) | 2026-06-02 | Version Corrective | Le domaine d'email personnalisé principal/unique peut désormais être supprimé (DELETE ne renvoie plus 400) |
| [v1.22.0](#v1-22-0) | 2026-05-30 | Version Mineure | Endpoint de suppression de demande de signature, événement webhook `signing_request.deleted` |
| [v1.21.1](#v1-21-1) | 2026-05-19 | Version Corrective | Activation du webhook de l'espace de travail via l'API |
| [v1.21.0](#v1-21-0) | 2026-05-14 | Version Mineure | Paramètre de partie locale de l'expéditeur d'email personnalisé |
| [v1.20.0](#v1-20-0) | 2026-05-12 | Version Mineure | Paramètre de contrôle de la navigation guidée |
| [v1.19.0](#v1-19-0) | 2026-05-12 | Version Mineure | Endpoints de gestion du logo, `icon_url` de l'espace de travail |
| [v1.18.0](#v1-18-0) | 2026-05-06 | Version Mineure | Champs modifiables par l'identité, données préremplies modifiables, validation de la valeur du champ |
| [v1.17.0](#v1-17-0) | 2026-05-01 | Version Mineure | Personnalisation des couleurs pour l'expérience de signature |
| [v1.16.0](#v1-16-0) | 2026-04-27 | Version Mineure | Endpoint de téléchargement de la demande de signature |
| v1.15.3 | 2026-04-27 | Mineure | Alias de champs propres |
| [v01.15.02](#v01-15-02) | 2026-04-23 | Version Corrective | Ajout des types de champ manquants au schéma des champs de create-and-send |
| [v01.15.01](#v01-15-01) | 2026-04-17 | Version Corrective | Ajout de la prise en charge des langues russe et polonaise aux schémas de l'API |
| [v01.15.00](#v01-15-00) | 2026-04-16 | Version Mineure | Webhooks d'espace de travail, endpoints de rotation/statut de secret |
| [v01.14.00](#v01-14-00) | 2026-04-14 | Version Mineure | Désignations Approbateur/CC, types de champ d'approbation |
| [v01.13.00](#v01-13-00) | 2026-04-12 | Version Mineure | Type de champ Tampon |
| [v01.12.01](#v01-12-01) | 2026-04-02 | Version Corrective | Noms de variables de champ personnalisé (`variable_defined_name`) |
| [v01.12.00](#v01-12-00) | 2026-03-26 | Version Mineure | Intégration du serveur MCP |
| [v01.11.00](#v01-11-00) | 2026-03-19 | Version Mineure | Champs de téléversement de fichier, balises d'ancrage, couleurs de fond de champ, changement de la valeur par défaut de l'ordre de signature |
| [v01.10.00](#v01-10-00) | 2026-03-10 | Version Mineure | Champs conditionnels, prise en charge DOCX, piste d'audit, contrôle du cadre de signature |
| [v01.09.00](#v01-09-00) | 2026-03-05 | Version Mineure | Vérification OTP, remplacement du document du modèle |
| [v01.08.00](#v01-08-00) | 2026-03-03 | Version Mineure | API des modèles d'email, prise en charge linguistique, observabilité des webhooks |
| [v01.07.00](#v01-07-00) | 2026-02-25 | Version Mineure | Signatures manuscrites, champs de paramètres legacy |
...
**Spécification OpenAPI :** [Télécharger la spécification v1.38.0](/api-reference/v01.38.00/openapi-v01.38.00.fr.json)

### Prise en charge du catalan

L'ensemble des langues prises en charge passe de 14 à 15 avec l'ajout du catalan (`ca`), qui rejoint `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro` et `nb`.

Le nouveau code est accepté sur toutes les surfaces de l'API porteuses de langue :

- **Paramètres de l'espace de travail** : `language` sur `PUT /workspace/{workspace_id}/settings` (et renvoyé par le GET correspondant)
- **Paramètres de l'entreprise** : `language` sur les endpoints de paramètres d'entreprise
- **Langue d'e-mail par demande de signature** : le champ `language` sur `POST /signing-requests`, `POST /signing-requests/create-and-send` et `PATCH` / `PUT /signing-requests/{id}`
- **Conditions du signataire** : le paramètre de chemin `{language}` sur les endpoints de conditions du signataire d'entreprise et d'espace de travail (`PUT` / `DELETE`)
- **Modèles d'e-mail par défaut** : `GET /email-templates/defaults/{language}`
- **Déclaration du cachet d'organisation** : `statement.language` lors de la création et de la mise à jour du cachet, renvoyé comme `statement_language` sur le cachet

Définir le catalan localise l'expérience complète du signataire : les e-mails destinés au signataire, l'interface de la cérémonie de signature, la déclaration de consentement enregistrée lors de l'acceptation des conditions et le format des dates. Le certificat de finalisation suit la langue de l'espace de travail : définissez le catalan sur l'espace de travail pour obtenir un certificat en catalan.

Les erreurs de validation pour les codes non pris en charge listent désormais les 15 langues.

#### Notes de migration

- La modification de l'API est additive. Les 14 codes de langue existants sont acceptés et renvoyés exactement comme avant, et les requêtes qui n'envoient jamais `ca` sont validées comme avant.
- Sauf si l'URL de signature comporte un code `lang` pris en charge, la page de signature sélectionne la première langue de la liste de préférences du navigateur du signataire prise en charge par Firma.dev. Un signataire dont le navigateur place `ca` ou une variante régionale `ca-` avant toute autre langue prise en charge voit désormais la page de signature en catalan. Avant la v1.38.0, le signataire voyait la langue suivante prise en charge dans la liste ou l'anglais. Les liens de signature envoyés par e-mail ne contiennent pas de paramètre `lang`, ce comportement s'applique donc aussi aux signataires existants. Un code `lang` pris en charge que vous ajoutez à une URL de signature intégrée définit toujours la langue ; toute autre valeur est ignorée.
- La déclaration de consentement est résolue dans cet ordre : les conditions de l'espace de travail dans la langue du signataire, les conditions de l'entreprise dans la langue du signataire, les conditions de l'espace de travail dans la langue configurée de l'espace de travail, les conditions de l'entreprise dans la langue configurée de l'entreprise, puis les conditions par défaut de Firma.dev dans la langue du signataire. S'il n'existe aucune condition en catalan ni dans la langue configurée applicable, un signataire qui donne la priorité au catalan accepte désormais la déclaration par défaut en catalan de Firma.dev au lieu de conditions personnalisées dans une autre langue du navigateur, par exemple l'espagnol. Les conditions dans la langue configurée de l'espace de travail ou de l'entreprise continuent de s'appliquer lorsqu'elles sont disponibles.
- Ajoutez des conditions du signataire en catalan au niveau de l'entreprise ou de l'espace de travail afin de conserver des conditions personnalisées pour ces signataires. Utilisez `PUT /company/signer-terms/ca`, `PUT /workspace/{workspace_id}/signer-terms/ca` ou les paramètres des conditions dans le tableau de bord.
...
**Spécification OpenAPI :** [Télécharger la spécification v1.37.0](/api-reference/v01.37.00/openapi-v01.37.00.fr.json) (inchangée dans cette version)

### Destinataires CC

Les endpoints de mise à jour enregistrent désormais les destinataires avec `"designation": "CC"`, et les endpoints de lecture les renvoient. Avant cette version, ils ne faisaient ni l'un ni l'autre.

#### Ajouter et mettre à jour des destinataires CC

- `PUT /templates/{id}` (`users[]`) et `PATCH /templates/{id}` (`user`) enregistrent les destinataires CC. Auparavant, ils renvoyaient `400` et `500`.
- `PUT /signing-requests/{id}` (`recipients[]`) et `PATCH /signing-requests/{id}` (`recipient`) enregistrent les destinataires CC. Auparavant, `PUT` renvoyait `200` et comptait le CC dans `summary.recipients_created` sans l'enregistrer, et `PATCH` renvoyait `500`.
- Sur `PUT /signing-requests/{id}`, un destinataire qui ne peut pas être enregistré apparaît désormais dans `details.recipients` de la réponse `400` au lieu d'être compté dans `summary.recipients_created`.
- Les destinataires CC existants peuvent être mis à jour par `id` et supprimés via `deleted_users` / `deleted_recipients`, comme les signataires et les approbateurs.
- `first_name` et `email` sont obligatoires pour un nouveau destinataire CC.
- `designation` est comparé sans tenir compte de la casse (`"CC"`, `"cc"`).

#### Destinataires CC dans les réponses

Les destinataires CC sont renvoyés avec `"designation": "CC"`, après les signataires et les approbateurs, par :

- `GET /signing-requests/{id}/users`
- `GET /templates/{id}/users`
- `GET /templates/{id}` et `GET /templates` (`recipients`)
- le tableau `recipients` des réponses de `POST /signing-requests`, `POST /signing-requests/create-and-send` et `PUT /signing-requests/{id}`

Les entrées CC ont les mêmes clés que les autres entrées de cet endpoint. Elles ne reçoivent jamais de lien de signature, et les clés qui ne concernent que la signature ont des valeurs neutres (par exemple `required_fields: []` et `ready_to_send: true`).

#### Création à partir d'un modèle

- `POST /signing-requests` et `POST /signing-requests/create-and-send` avec `template_id` copient désormais les destinataires CC du modèle vers la nouvelle demande de signature. Un CC du modèle est ignoré lorsque la demande a déjà un CC avec la même adresse e-mail (sans tenir compte de la casse).
- Un destinataire du corps de la requête avec `"designation": "CC"` est toujours ajouté comme destinataire CC. Il n'est jamais associé à un utilisateur du modèle par `template_user_id` ou `order`.

#### Validation

- Faire passer un destinataire existant de CC à Signer ou Approver, ou l'inverse, renvoie `400`. `PATCH /templates/{id}` et `PATCH /signing-requests/{id}` renvoient le message `Cannot change designation between CC and Signer/Approver; delete the recipient and create it again`, `PUT /signing-requests/{id}` l'indique dans `details.recipients` et `PUT /templates/{id}` renvoie le message générique `Validation errors occurred`. Supprimez le destinataire et recréez-le avec la nouvelle désignation.
- Un champ ne peut pas être attribué à un destinataire CC. Sur `PUT /signing-requests/{id}`, y compris pour un CC créé dans la même requête via `_temp_id`, l'erreur du champ indique `Cannot assign fields to CC recipient.` ; les autres chemins de champs rejettent l'id d'un CC comme destinataire invalide.
- La mise à jour ou la suppression d'un destinataire par `id` ne trouve désormais que les destinataires non supprimés. Un destinataire déjà supprimé est traité comme introuvable au lieu d'être modifié.

#### Notes de migration

- Aucun nouvel endpoint, champ ou valeur d'enum : la spécification v1.37.0 reste valable.
- Les listes de destinataires peuvent désormais contenir des entrées CC. Si votre code traite chaque entrée comme un signataire, vérifiez `designation`.
- Les demandes de signature créées à partir d'un modèle qui a des destinataires CC incluent désormais ces destinataires CC, qui reçoivent le document finalisé. Retirez les destinataires CC du modèle si vous ne souhaitez pas ce comportement.
- Si vous comptiez sur une réponse `200` de `PUT /signing-requests/{id}` lorsqu'un destinataire n'était pas enregistré, gérez la réponse `400`.
...
**Spécification OpenAPI :** [Télécharger la spécification v1.37.0](/api-reference/v01.37.00/openapi-v01.37.00.fr.json)

### Cachets d'organisation

Cachets électroniques appliqués automatiquement au nom d'une entreprise, en tant que participants dans l'ordre de signature. La configuration, les règles de portée et le rendu sur le certificat sont décrits dans le [guide des Cachets d'Organisation](/guides/organization-seals).

#### Nouveaux endpoints

| Méthode | Chemin | Objet |
|---|---|---|
| `POST` | `/seals` | Créer un cachet (`kind` : `uploaded`, `typed` ou `drawn` ; `scope` : `company` ou `workspace` avec `workspace_id`). Clé API protégée requise pour la portée entreprise |
| `GET` | `/seals` | Lister les cachets visibles dans la portée de la clé |
| `GET` | `/seals/{id}` | Obtenir un cachet |
| `PATCH` | `/seals/{id}` | Remplacer l'image (nouvelle version), renommer, mettre à jour `display_name` / `signatory_title` ou définir `is_default` |
| `DELETE` | `/seals/{id}` | Révoquer ; ajoutez `?stop_pending=true` pour mettre en pause les participants en transit |
| `GET` | `/seals/{id}/image` | PNG canonique ; chaque lecture est journalisée |
| `GET` | `/seals/{id}/applications` | Demandes de signature auxquelles le cachet a été appliqué |
| `GET` | `/seals/{id}/access-log` | Événements de cycle de vie et de lecture d'image |
| `DELETE` | `/seals/{id}/erase` | Effacer un lignage révoqué référencé par aucune demande ni modèle, 90 jours après la révocation |
| `DELETE` | `/signing-requests/{id}/seal-participants/{participant_id}` | Retirer un participant de cachet et ses champs d'une demande non envoyée |

#### Participants de cachet

Les demandes de signature et les modèles acceptent un tableau `seal_participants[]` aux côtés de `recipients[]` / `users[]` sur `POST /signing-requests`, `POST /signing-requests/create-and-send`, `PATCH /signing-requests/{id}`, `POST /templates` et `PATCH /templates/{id}`. Chaque entrée comprend `temp_id` (identifiant client servant à affecter les champs), `seal_id` et `order`. Destinataires et cachets partagent un espace d'ordre contigu unique ; l'ordre 1 s'applique à l'envoi.

Les champs appartenant à un cachet portent `seal_participant_temp_id` au lieu d'un destinataire. Les champs de cachet sont générés par le serveur (`read_only: 1`, sans destinataire) et les valeurs fournies par le client sur ces champs sont ignorées. Les réponses des demandes de signature incluent désormais `seal_participants[]`.

#### Échange de cachet

`PATCH /signing-requests/{id}` accepte `seal_participant: { "id": "<participant-id>", "swap_to_seal_id": "<seal-uuid>" }` pour déplacer un participant en pause d'une demande envoyée vers un autre cachet de la portée. Clé API protégée requise.

#### Événements webhook

| Événement | Déclencheur |
|---|---|
| `organization_seal.created` | Cachet créé |
| `organization_seal.updated` | Image remplacée (nouvelle version), renommé ou statut par défaut modifié |
| `organization_seal.deleted` | Cachet révoqué |
| `organization_seal.erased` | Lignage révoqué définitivement effacé |
| `signing_request.seal.applied` | Cachet appliqué à un document |
| `signing_request.seal.paused` | Participant de cachet mis en pause (révoqué avec arrêt des applications en cours, ou incohérence d'intégrité) |
| `signing_request.seal.swapped` | Participant en pause échangé contre un autre cachet |

#### Codes d'erreur

`SEALS_DISABLED`, `SEAL_NOT_FOUND`, `SEAL_SCOPE_FORBIDDEN`, `SEAL_IMAGE_INVALID`, `SEAL_ALREADY_REVOKED`, `SEAL_ERASE_NOT_ELIGIBLE`, `SEAL_MUTATION_NOT_ALLOWED`, `SEAL_ORDER_COLLISION`, `SEAL_PAUSED`, `SEAL_UNAVAILABLE` et `SEAL_CREATION_DISABLED_EDGE` (réservé à l'hôte edge de repli ; l'hôte principal de l'API effectue la création des cachets).

#### Notes de migration

- Purement additif. `recipients[]`, `users[]` et tous les endpoints existants sont inchangés.
...
**Date de sortie :** 2026-08-31

**Type de version :** Version mineure

**Spécification OpenAPI :** [Télécharger la spécification v1.36.0](/api-reference/v01.36.00/openapi-v01.36.00.fr.json)

### Anchor tags

- `add_white_background` dans les définitions d'anchor tags : peint un fond blanc derrière le champ ancré.

### Réponses d'erreur

- Les objets d'erreur portent un `code` lisible par machine.
- Les échecs de validation renvoient aussi `errors[]` avec les erreurs par champ.

### Create and send

- Les réponses de `POST /signing-requests/create-and-send` incluent `warnings[]` avec des avertissements non bloquants.

#### Notes de migration

- Purement additif. Aucun changement incompatible.

---

## v1.35.0

**Date de publication :** 13 aout 2026

**Type de version :** Version Mineure

**Specification OpenAPI :** [Telecharger la spec v1.35.0](/api-reference/v01.35.00/openapi-v01.35.00.fr.json)

### Copie de modele entre espaces de travail

Nouveau endpoint `POST /templates/{id}/copy` qui copie un modele vers un autre espace de travail au sein de la meme entreprise. Toutes les donnees associees sont copiees avec un remappage correct des IDs :

- Parametres du modele (expiration, ordre de signature, e-mails, etc.)
- Destinataires (signataires, approbateurs) avec designations preservees
- Champs avec references `required_conditions` et `visibility_conditions` remappees
- Destinataires en copie (CC)
- Definitions de champs personnalises
- Rappels avec references aux destinataires remappees
- Document PDF (copie independante)

**Corps de la requete :**

- `workspace_id` (requis, uuid) - espace de travail cible
- `name` (optionnel, string, max 255) - nom de la copie (par defaut, nom original avec suffixe " (copy)")

**Autorisation :** necessite une cle API de niveau entreprise (protegee). Les cles d'espace de travail recoivent `403 PROTECTED_KEY_REQUIRED`.
...
[Télécharger la spécification OpenAPI](/api-reference/v01.33.00/openapi-v01.33.00.json)

### Cinq nouvelles langues

L'ensemble des langues prises en charge passe de 9 à 14 : le tchèque (`cs`), le suédois (`sv`), le néerlandais (`nl`), le roumain (`ro`) et le norvégien bokmål (`nb`) rejoignent `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`.

Les nouveaux codes sont acceptés sur toutes les surfaces de l'API porteuses de langue :

- **Paramètres de l'espace de travail** — `language` sur `PUT /workspace/{workspace_id}/settings` (et renvoyé par le GET correspondant)
- **Paramètres de l'entreprise** — `language` sur les endpoints de paramètres d'entreprise
- **Langue d'e-mail par demande de signature** — le champ `language` sur `POST /signing-requests`, `POST /signing-requests/create-and-send` et `PATCH` / `PUT /signing-requests/{id}`
- **Conditions du signataire** — le paramètre de chemin `{language}` sur les endpoints de conditions du signataire d'entreprise et d'espace de travail (`GET` / `PUT` / `DELETE`)
- **Modèles d'e-mail par défaut** — `GET /email-templates/defaults/{language}`

Définir l'une des nouvelles langues localise l'expérience complète du signataire : les e-mails destinés au signataire, l'interface de la cérémonie de signature, la déclaration de consentement enregistrée lors de l'acceptation des conditions, le format des dates et le certificat de finalisation (y compris le rendu correct des diacritiques tchèques et roumains).

Les erreurs de validation pour les codes non pris en charge listent désormais les 14 langues.

### Guide de migration

Aucune action requise. Le changement est purement additif : les 9 codes de langue existants se comportent exactement comme avant, et les espaces de travail ou demandes de signature qui n'ont jamais défini de langue ne sont pas affectés.
...
[Télécharger la spécification OpenAPI](/api-reference/v01.32.00/openapi-v01.32.00.fr.json)

### Taille de police explicite par champ

Les champs textuels (`text`, `textarea`, `email`, `name`, `phone`, `company`, `title`, `number`, `date`, `signing_date`, `approval_date`, `dropdown`, `url`) acceptent désormais un `fontSize` optionnel dans `format_rules` — un entier entre 8 et 48, en pixels. Voir le nouveau schéma `TextFormatRules` ; `DateFormatRules` reçoit la même propriété optionnelle, un champ de date peut donc porter à la fois `dateFormat` et `fontSize`.

La valeur est une taille **initiale/maximale**, pas une garantie : la vue de signature et le PDF scellé commencent à afficher la valeur du champ à la taille explicite, puis la réduisent automatiquement pour tenir dans le cadre du champ si le contenu déborde. Omettez la propriété pour un dimensionnement automatique (le comportement précédent, inchangé). Les valeurs hors plage sont ramenées à 8-48 ; les valeurs non numériques sont ignorées. Les champs signature, initials, checkbox, radio, file et stamp n'acceptent pas de taille de police.

Définissez-la partout où `format_rules` est accepté — création/mise à jour des champs de modèle et payloads de champs des demandes de signature — par exemple :
...
Les anchor tags reçoivent une propriété optionnelle `font_size` (entier, 8-48). Lorsqu'un anchor est résolu en champ, la valeur est enregistrée sur le champ créé sous `format_rules.fontSize` et se comporte exactement comme décrit ci-dessus.

### Guide de migration

Aucune action requise. Les propriétés sont optionnelles et additives — les champs sans `fontSize` s'affichent exactement comme avant, et les modèles et demandes de signature existants ne sont pas affectés.
...
Nouveau champ optionnel `language` sur les endpoints de création et de mise à jour de demandes de signature : `POST /signing-requests`, `POST /signing-requests/create-and-send` et `PATCH` / `PUT /signing-requests/{id}`. Définissez-le sur l'une des valeurs `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl` et tous les e-mails destinés au signataire de cette demande de signature — invitation, rappels, vérification OTP, finalisation et CC, expiration, annulation et refus — ainsi que leur format de date, utilisent cette langue.
