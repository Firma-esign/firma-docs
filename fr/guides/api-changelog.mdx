...
- Lorsque l'URL de signature ne contient pas de paramètre `lang`, la page de signature sélectionne la première langue de la liste de préférences du navigateur du signataire prise en charge par Firma.dev. Un signataire dont le navigateur place `ca` ou une variante régionale `ca-` avant toute autre langue prise en charge voit désormais la page de signature en catalan. Avant la v1.38.0, le signataire voyait la langue suivante prise en charge dans la liste ou l'anglais. Les liens de signature envoyés par e-mail ne contiennent pas de paramètre `lang`, ce comportement s'applique donc aussi aux signataires existants. Un paramètre `lang` que vous ajoutez à une URL de signature intégrée définit toujours la langue.
