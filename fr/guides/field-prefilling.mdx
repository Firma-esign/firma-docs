...
| :--- | :--- | :--- | :--- |
| `variable_name` | Propriété de champ de premier niveau (requête et réponse) | Un libellé que vous attribuez à un champ. Utilisé comme critère de correspondance de repli lors de la fusion de champs dans une demande de signature créée à partir d'un modèle. **Ne lit ni n'écrit jamais les données du destinataire.** | Non |
| `read_only_value` | Propriété de champ de premier niveau (requête) | Texte statique que vous fournissez à la création du champ. **Ne peut pas être combinée avec `format_rules.prefilledData` sur le même champ** — l'API renvoie 400 si les deux sont définis. Lorsqu'un modèle porte déjà une règle `prefilledData`, `read_only_value` sur le champ de la requête remplace la règle du modèle lors de la fusion. | Oui — l'emporte toujours quand elle est présente |
| `format_rules.prefilledData` | Imbriquée dans `format_rules` sur le champ | La clé d'attribut du destinataire à partir de laquelle remplir automatiquement (`email`, `full_name`, `phone_number`, `company`, etc., ou une clé `custom_fields` du destinataire). Résolue au moment de l'envoi à partir des données du destinataire assigné. **Ne peut pas être combinée avec `read_only_value` sur le même champ** — l'API renvoie 400. | Oui — en l'absence de `read_only_value` |
