...
Comme le JWT est généré pour un modèle spécifique (et que ce modèle appartient à un espace de travail spécifique), la frontière entre clients est imposée par le token lui-même - un token émis pour le modèle du Client A ne peut pas être rejoué contre les données du Client B. Les éditeurs intégrés héritent de la palette de couleurs de l'espace de travail une fois configurée - si aucune couleur personnalisée n'est définie, ils reviennent à la palette par défaut de Firma. Définissez les champs `color_*` dans les paramètres de l'espace de travail pour appliquer la marque de votre client. Un token correctement délimité vous procure l'isolation entre clients ; l'ajout de couleurs personnalisées complète la marque blanche.
