...
Firma parle désormais 15 langues. Le catalan (`ca`) rejoint les quatorze langues existantes sur l'ensemble de la plateforme : le tableau de bord, l'expérience de signature côté signataire, tous les e-mails aux signataires, les conditions de consentement du signataire, le format des dates, les certificats de finalisation et la déclaration du cachet d'organisation ainsi que ses e-mails de cycle de vie.

- Définissez la langue par espace de travail, par entreprise ou par demande de signature individuelle, depuis le tableau de bord ou via l'API. Ce réglage détermine les e-mails d'invitation et de rappel et le format des dates. Le certificat de finalisation suit la langue de l'espace de travail
- Sauf si l'URL de signature comporte un code `lang` pris en charge, la cérémonie de signature et la déclaration de consentement enregistrée lors de l'acceptation des conditions suivent la langue du navigateur du signataire, détectée automatiquement ; un code `lang` pris en charge dans l'URL de signature définit la langue à la place
- Les certificats affichent correctement les diacritiques catalans et le l géminé (l·l) grâce aux polices intégrées
- Les navigateurs déclarant une variante régionale ou dialectale du catalan (`ca-ES`, `ca-AD`, `ca-FR`, `ca-IT`, `ca-valencia`) sont automatiquement détectés comme `ca`

Consultez le [changelog de l'API](/guides/api-changelog#v1-38-0) pour les enums de langue élargis.
...
Automatisez la contresignature de votre entreprise. Un cachet d'organisation appose un cachet d'entreprise, un nom d'affichage et le titre du signataire sur le PDF finalisé en tant que participant dans l'ordre de signature, et le certificat de finalisation l'enregistre comme « Appliqué automatiquement ».

- **Portées entreprise et espace de travail** : gérez les cachets dans **Paramètres > Cachets d'organisation** (entreprise) ou dans **Paramètres > Cachets d'organisation** d'un espace de travail, avec un défaut par portée et possibilité de changement par l'expéditeur
- **Trois modes de création** : téléchargez un PNG, saisissez le nom de l'entreprise ou dessinez une marque, chacun avec une attestation d'autorité enregistrée
- **Placement à n'importe quelle position de l'ordre** : ajoutez un cachet depuis le panneau **Utilisateurs du modèle** ou **Signataires** ; la première position s'applique à l'envoi, toute autre position s'applique une fois les participants précédents terminés
- **Révoquer et échanger** : révoquez de manière prospective, arrêtez éventuellement les applications en cours et échangez un cachet en pause contre un autre depuis la vue de la demande
- **API et webhooks** : `seal_participants[]` sur les modèles et demandes de signature, endpoints de gestion `/seals` et sept événements webhook

Consultez le [guide des Cachets d'Organisation](/guides/organization-seals) pour la configuration et le [journal des modifications de l'API](/guides/api-changelog#v1-37-0) pour les détails des endpoints.

---

## 3 août 2026

### Cinq nouvelles langues : tchèque, suédois, néerlandais, roumain et norvégien

Firma parle désormais 14 langues. Le tchèque (`cs`), le suédois (`sv`), le néerlandais (`nl`), le roumain (`ro`) et le norvégien bokmål (`nb`) rejoignent les neuf langues existantes sur l'ensemble de la plateforme : le tableau de bord, l'expérience de signature côté signataire, tous les e-mails aux signataires, les conditions de consentement du signataire, le format des dates et les certificats de finalisation.

- Définissez la langue par espace de travail, par entreprise ou par demande de signature individuelle — depuis le tableau de bord ou via l'API
- L'ensemble du parcours du signataire est localisé : e-mails d'invitation et de rappel, la cérémonie de signature, la déclaration de consentement enregistrée lors de l'acceptation des conditions et le certificat de finalisation
- Les certificats affichent correctement les diacritiques tchèques et roumains grâce aux polices intégrées
- Les navigateurs déclarant le norvégien comme `no` ou `nn` sont automatiquement détectés comme `nb`

Consultez le [changelog de l'API](/fr/guides/api-changelog#v1330) pour les enums de langue élargis.
