...
| | `POST /signing-requests` (création) | `POST /signing-requests/create-and-send` |
| :--- | :--- | :--- |
| Résultat | Un **brouillon** (`status: "draft"`). Rien n'est envoyé. | La demande est créée **et** envoyée, de manière atomique, en un seul appel. |
| Destinataires notifiés ? | Jamais - quel que soit `send_signing_email` ou tout autre paramètre. | Par défaut, oui. Voir `send_signing_email` ci-dessous. |
| Crédit facturé / `sent_on` défini ? | Non | Oui, immédiatement. |
| Toujours modifiable via PATCH/PUT ? | Oui, jusqu'à ce que vous appeliez `/send` | Non - la demande est déjà dans un état envoyé. |
| Idéal pour | Un workflow de type **brouillon puis édition** : construire la demande, ajuster les destinataires ou les champs, la faire relire en interne, puis l'envoyer une fois prête. | Un appel **envoyer et oublier** lorsque vous avez déjà tout ce dont vous avez besoin. |

Utilisez [`create`](/api-reference/v01.33.00/signing-requests/create-signing-request) lorsque votre workflow nécessite une étape de relecture ou d'édition avant qu'un destinataire ne soit atteint. Utilisez [`create-and-send`](/api-reference/v01.33.00/signing-requests/create-and-send-signing-request-atomic) lorsque vous voulez que la demande soit active et les destinataires notifiés en un seul appel.
...
| Endpoint | Effet de `send_signing_email: false` |
| :--- | :--- |
| `POST /signing-requests` (création) | Aucun. Ce endpoint n'envoie jamais d'email de lui-même, donc le paramètre est simplement stocké pour plus tard - il ne prend effet qu'une fois le brouillon réellement envoyé. |
| `POST /signing-requests/create-and-send` | La demande est quand même créée **et** envoyée : le crédit est facturé, `sent_on` est défini, le webhook `signing_request.sent` se déclenche, et le lien de signature est actif. Seul l'**email** de notification au destinataire est supprimé. |
...
<Tip>
  Vous voulez qu'un champ affiche une valeur avant que le signataire ne le touche - une valeur fixe que vous connaissez déjà, ou les propres données de profil du destinataire ? Consultez le [guide de préremplissage des champs](/guides/field-prefilling). `variable_name` seul ne fait **pas** cela.
</Tip>
...
Affichez cette URL dans un iframe. Pour la configuration complète de l'intégration - y compris les événements postMessage, le dimensionnement de l'iframe et les autorisations caméra/presse-papiers - consultez le [guide Signature intégrable](/guides/embeddable-signing).
...
- [Préremplissez les champs avec les données du destinataire ou des données statiques](/guides/field-prefilling) avant l'envoi
