...
title: "Webhooks"
icon: "webhook"
description: "Implémentez des intégrations webhook sécurisées pour recevoir des notifications d'événements en temps réel depuis Firma."
---


Firma envoie des événements webhook pour notifier votre application des changements de cycle de vie de signature, des complétions de documents et des activités d'espace de travail. Les webhooks permettent des intégrations en temps réel sans interrogation.

## Cas d'usage courants

- Envoyer des notifications internes lorsque des documents sont signés
- Mettre à jour votre base de données lorsque les demandes de signature sont terminées
...
- `signing_request.viewed` - Un destinataire a consulté le document
- `signing_request.completed` - Tous les destinataires ont terminé la signature
- `signing_request.expired` - La demande de signature a expiré
- `signing_request.cancelled` - La demande de signature a été annulée
- `signing_request.updated` - Les métadonnées de la demande de signature ont été mises à jour
...
- `signing_request.recipient.identity_changed` - Le destinataire a modifié son identité (nom, entreprise, etc.) pendant la signature
...
- `domain.verification.failed` - La vérification du domaine a échoué
...
<Note>
  Votre URL de webhook doit utiliser HTTPS et répondre en moins de 5 secondes. Firma enverra un événement de test lors de la création pour vérifier l'endpoint.
</Note>
...
## Sécurité : vérification des signatures (requis)

<Warning>
  **Vérifiez toujours les signatures de webhook** pour empêcher les attaques par usurpation. Ne traitez pas les webhooks sans vérification de signature.
</Warning>

Firma signe toutes les requêtes webhook en utilisant HMAC SHA-256. Votre endpoint de webhook reçoit ces en-têtes :
...
- `X-Firma-Delivery` - ID unique de tentative de livraison
...
- `t` = horodatage Unix (secondes) lorsque la signature a été générée
- `v1` = digest hex HMAC-SHA256
- Format de charge utile signée : `{timestamp}.{json_body}`
...
3. Stockez le secret en toute sécurité (variable d'environnement ou gestionnaire de secrets)
...
## Gestion de la rotation du secret

Lorsque vous faites tourner votre secret de signature de webhook :
...
3. Après 24 heures, seul `X-Firma-Signature` est envoyé

**Implémentation** : vérifiez d'abord `X-Firma-Signature`. Si la vérification échoue et que `X-Firma-Signature-Old` existe, vérifiez avec l'ancien secret.

## Comportement des nouvelles tentatives

Firma réessaye automatiquement les livraisons de webhook échouées :

- **Planning des nouvelles tentatives** : 1 minute, 5 minutes, 30 minutes, 2 heures, 6 heures
- **Tentatives totales** : jusqu'à 5 nouvelles tentatives par événement
- **Timeout** : votre endpoint doit répondre en moins de 5 secondes
- **Succès** : tout code de statut 2xx indique le succès
- **Désactivation automatique** : après 50 échecs consécutifs, le webhook est automatiquement désactivé

<Tip>
  **Bonne pratique** : répondez avec 200 immédiatement, puis traitez les événements de manière asynchrone (file d'attente, job en arrière-plan, etc.) pour éviter les timeouts.
</Tip>

## Idempotence

Gérez toujours les événements dupliqués en utilisant l'`id` d'événement :
...
## Surveillance de la santé du webhook

Surveillez la santé de votre webhook en vérifiant les détails du webhook :
...
- `consecutive_failures` - Nombre de livraisons échouées consécutives
- `last_failure_at` - Horodatage de l'échec le plus récent
- `enabled` - Si le webhook est actif (désactivé automatiquement après 50 échecs)
...
- Vérifiez que vous hachez le corps de requête brut (pas le JSON analysé)
- Assurez-vous que vous utilisez HMAC SHA-256, pas d'autres algorithmes de hachage

**Timeouts / erreurs 504**

- Répondez avec 200 immédiatement, traitez de manière asynchrone
- Vérifiez que votre endpoint répond en moins de 5 secondes
- Utilisez des jobs/files d'attente en arrière-plan pour les traitements lourds

**Événements dupliqués**

- Implémentez l'idempotence en utilisant l'`id` d'événement
- Stockez les ID d'événements traités dans votre base de données

**Webhook désactivé automatiquement**

- Vérifiez `consecutive_failures` et les logs d'événements récents
- Corrigez les problèmes d'endpoint, puis réactivez le webhook via l'API ou le tableau de bord
...
- Vérifiez les signatures HMAC sur toutes les requêtes webhook
- Gérez la rotation de signature (vérifiez à la fois `X-Firma-Signature` et `X-Firma-Signature-Old`)
- Répondez avec 200 en moins de 5 secondes
- Traitez les événements de manière asynchrone (files d'attente/jobs en arrière-plan)
- Implémentez l'idempotence en utilisant l'`id` d'événement
- Stockez le secret de signature en sécurité (variable d'environnement ou gestionnaire de secrets)
- Surveillez la métrique `consecutive_failures` via l'endpoint GET du webhook
- Configurez des alertes pour les échecs de webhook
- Journalisez tous les événements webhook pour le débogage
- Testez avec tous les types d'événements auxquels vous êtes abonné
- Restez dans les limites de débit (Consultez le [Guide des limites de débit](rate-limits))
