...
| --- | --- | --- |
| `enabled` par webhook | Un abonnement webhook individuel | Défini sur le webhook lui-même (`PATCH /webhooks/{id}`) |
...
<Note>
  **Limite de débit** : Les opérations de lecture (GET) des webhooks prennent en charge **200 requêtes par minute par clé API**. Les opérations d'écriture des webhooks (création, mise à jour, suppression) sont limitées à **60 requêtes par minute**.
</Note>
