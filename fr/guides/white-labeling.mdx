...
<Note>
  Si le bouton du tableau de bord d'un espace de travail pour ce paramètre affiche un message de succès mais que la valeur ne change pas réellement, il s'agit d'un problème de permissions, pas d'un bug : seuls les administrateurs de l'entreprise ou de l'espace de travail peuvent modifier ce paramètre depuis l'interface du tableau de bord. Les membres de l'équipe sans accès administrateur doivent demander à un administrateur de le mettre à jour, ou utiliser directement l'API avec votre clé API, qui n'est pas soumise à la restriction administrateur du tableau de bord.
</Note>
