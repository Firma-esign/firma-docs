...
| `color_accent` | Couleur d'accentuation secondaire | `#34eeff` |
| `color_accent_fg` | Couleur du texte sur les éléments d'accentuation | `#000000` |
| `color_canvas` | Arrière-plan du canevas/visionneuse de document | `#0f1419` |
| `color_muted` | Zones d'arrière-plan atténuées/subtiles | `#22222a` |
| `color_muted_fg` | Couleur du texte sur les zones atténuées | `#b8b8b8` |
...
| `show_qr_code` | Inclure un code QR renvoyant vers la page de signature dans les emails | `true` |
...
Langues prises en charge : `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro`, `nb`, `ca`.
...
Consultez la [référence de l'API des domaines d'email](/api-reference/v01.27.00/email-domains/list-company-domains) pour la documentation complète des points de terminaison, ou [Domaines personnalisés](/guides/custom-domains) pour les détails des enregistrements DNS, la résolution des conflits avec Resend, et le dépannage de l'état de vérification.
...
**Le nom de l'expéditeur** est le nom de l'espace de travail (ou le nom de l'entreprise si l'espace de travail n'a pas de nom défini).

Par exemple, si votre espace de travail s'appelle « Acme Legal » et que vous définissez `email_local_part` sur `noreply` avec un domaine vérifié `sign.acmecorp.com`, les emails seront envoyés depuis :
...
[Guide de l'éditeur de modèles intégrable](embeddable-template-editor) - Génération de JWT, événements et implémentation complète
