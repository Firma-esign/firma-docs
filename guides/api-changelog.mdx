...
- When the signing URL has no `lang` parameter, the signing page selects the first language in the signer's browser preference list that Firma.dev supports. A signer whose browser lists `ca` or a `ca-` regional tag ahead of every other supported language now sees the signing page in Catalan. Before v1.38.0, the signer saw the next supported language in the list or English. Signing links sent by email do not include a `lang` parameter, so this behavior also applies to existing signers. A `lang` parameter you add to an embedded signing URL still sets the language.
