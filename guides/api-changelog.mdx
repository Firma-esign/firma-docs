...
New optional `language` field on `POST /signing-requests/create-and-send` and `PATCH /signing-requests/{id}`. Set it to one of `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, and every signer-facing email for that signing request — invitation, reminders, OTP verification, completion and CC, expiry, cancellation, and decline — along with its date formatting, uses that language.
