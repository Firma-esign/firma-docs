...
|---------|------|--------------|-------------|
| [v1.38.0](#v1-38-0) | 2026-09-17 | Minor Release | Catalan (`ca`) support: the platform's supported language set grows to 15, accepted everywhere a language code is accepted, including workspace and company settings, per-signing-request email language, signer terms, email template defaults, and the organization-seal statement language |
| [v1.37.1](#v1-37-1) | 2026-09-15 | Patch Release | CC recipients: accepted on `PUT`/`PATCH` of templates and signing requests, returned in every recipient list, and copied from the template on template-based creation |
| [v1.37.0](#v1-37-0) | 2026-09-14 | Minor Release | Organization seals: seal lifecycle endpoints under `/seals`, `seal_participants[]` on templates and signing requests, seal swap on sent requests, seven new webhook events |
| [v1.36.0](#v1-36-0) | 2026-08-31 | Minor Release | Anchor tags: `add_white_background`; structured error responses (`code`, `errors[]`); `warnings[]` on create-and-send responses |
| [v1.35.0](#v1-35-0) | 2026-08-13 | Minor Release | Cross-workspace template copy: new `POST /templates/{id}/copy` endpoint deep-copies a template to another workspace (protected API key required) |
| [v1.34.0](#v1-34-0) | 2026-08-07 | Minor Release | Completion page customization: custom title, message, redirect URL, and redirect delay at four cascading levels (company, workspace, template, signing request) |
| [v1.33.0](#v1-33-0) | 2026-08-03 | Minor Release | Five new languages — Czech (`cs`), Swedish (`sv`), Dutch (`nl`), Romanian (`ro`), Norwegian Bokmål (`nb`) — accepted everywhere a language code is accepted: workspace/company settings, per-signing-request email language, signer terms, and email template defaults |
| [v1.32.0](#v1-32-0) | 2026-07-22 | Minor Release | Explicit field font size: optional `fontSize` (8-48px) in `format_rules` for text-bearing fields, honored in the signing view and the sealed PDF with auto-shrink preserved; new `font_size` property on anchor tags |
| [v1.31.0](#v1-31-0) | 2026-07-14 | Minor Release | Opt-in per-signing-request email language: new `language` field on create-and-send and update sets the language for all signer-facing emails (falls back to workspace/company default when omitted) |
| [v1.30.0](#v1-30-0) | 2026-07-08 | Minor Release | Signer mark retrieval: signature / initials / stamp images (base64 PNG) and signer-uploaded file download, per signer, via the API |
| [v1.29.0](#v1-29-0) | 2026-07-06 | Minor Release | Two-step document upload (`POST /documents`) for large files up to 50MB; new `document_id` field on create and create-and-send |
| [v1.28.0](#v1-28-0) | 2026-07-05 | Minor Release | TypeScript SDK code samples (`@firma-dev/sdk`) added to every endpoint; embedded signing-request and legacy `/documents` endpoints removed from the reference |
| [v1.27.0](#v1-27-0) | 2026-06-29 | Patch Release | Settings schemas documented: signing-request responses now expose `show_qr_code`, `allow_presigning_download`, `identity_editable_fields`, `notify_identity_change_email`; template responses expose `show_qr_code`; `cancel` returns `emails_sent`; `resend` returns `recipients_failed`; new `TemplateSettings` and `LegacyDocumentSettings` schemas |
| [v1.26.0](#v1-26-0) | 2026-06-23 | Minor Release | Workspace settings: signing button label overrides, QR code toggle, presigning download toggle, partial watermark toggle |
| [v1.25.2](#v1-25-2) | 2026-06-18 | Patch Release | Provided fields that match no template field now return `400` (previously ignored silently) on `POST /signing-requests` and `/create-and-send`; field `type` values are matched case-insensitively |
| [v1.25.1](#v1-25-1) | 2026-06-14 | Patch Release | `create-and-send` send-step failures now return the real status (`400`/`402`/`422`) with a machine-readable `code`, instead of masking them as `500` |
| [v1.25.0](#v1-25-0) | 2026-06-14 | Minor Release | Editor brand colors: 5 new appearance fields (`color_accent`, `color_accent_fg`, `color_canvas`, `color_muted`, `color_muted_fg`) on company + workspace settings; resolved `color_palette` now returns 11 keys |
| [v1.24.0](#v1-24-0) | 2026-06-09 | Minor Release | Workspace test API keys: create returns a live+test pair, reads return both, key_type-aware regenerate/expire |
| [v1.23.0](#v1-23-0) | 2026-06-04 | Minor Release | Signer terms endpoints, company settings endpoints, terms acceptance settings |
| [v1.22.1](#v1-22-1) | 2026-06-02 | Patch Release | Primary/only custom email domain can now be deleted (DELETE no longer returns 400) |
| [v1.22.0](#v1-22-0) | 2026-05-30 | Minor Release | Delete signing request endpoint, signing_request.deleted webhook event |
| [v1.21.1](#v1-21-1) | 2026-05-19 | Patch Release | Workspace webhook activation via API |
| [v1.21.0](#v1-21-0) | 2026-05-14 | Minor Release | Custom email sender local-part setting |
| [v1.20.0](#v1-20-0) | 2026-05-12 | Minor Release | Guided navigation control setting |
| [v1.19.0](#v1-19-0) | 2026-05-12 | Minor Release | Logo management endpoints, workspace icon_url |
| [v1.18.0](#v1-18-0) | 2026-05-06 | Minor Release | Identity editable fields, editable prefilled data, field value validation |
| [v1.17.0](#v1-17-0) | 2026-05-01 | Minor Release | Color customization for signing experience |
| [v1.16.0](#v1-16-0) | 2026-04-27 | Minor Release | Download Signing Request endpoint |
| v1.15.3 | 2026-04-27 | Minor | Clean field aliases |
| [v01.15.02](#v01-15-02) | 2026-04-23 | Patch Release | Added missing field types to create-and-send fields schema |
| [v01.15.01](#v01-15-01) | 2026-04-17 | Patch Release | Added Russian and Polish language support to API schemas |
| [v01.15.00](#v01-15-00) | 2026-04-16 | Minor Release | Workspace Webhooks, Secret Rotation/Status Endpoints |
| [v01.14.00](#v01-14-00) | 2026-04-14 | Minor Release | Approver/CC Designations, Approval Field Types |
| [v01.13.00](#v01-13-00) | 2026-04-12 | Minor Release | Stamp Field Type |
| [v01.12.01](#v01-12-01) | 2026-04-02 | Patch Release | Custom Field Variable Names (`variable_defined_name`) |
| [v01.12.00](#v01-12-00) | 2026-03-26 | Minor Release | MCP Server Integration |
| [v01.11.00](#v01-11-00) | 2026-03-19 | Minor Release | File Upload Fields, Anchor Tags, Field Background Colors, Signing Order Default Change |
| [v01.10.00](#v01-10-00) | 2026-03-10 | Minor Release | Conditional Fields, DOCX Support, Audit Trail, Signature Frame Control |
| [v01.09.00](#v01-09-00) | 2026-03-05 | Minor Release | OTP Verification, Replace Template Document |
| [v01.08.00](#v01-08-00) | 2026-03-03 | Minor Release | Email Templates API, Language Support, Webhook Observability |
| [v01.07.00](#v01-07-00) | 2026-02-25 | Minor Release | Hand-Drawn Signatures, Legacy Settings Fields |
...
**OpenAPI Spec:** [Download v1.38.0 spec](/api-reference/v01.38.00/openapi-v01.38.00.json)

### Catalan support

The supported language set grows from 14 to 15 with the addition of Catalan (`ca`), joining `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro`, and `nb`.

The new code is accepted at every language-bearing surface of the API:

- **Workspace settings**: `language` on `PUT /workspace/{workspace_id}/settings` (and returned by the corresponding GET)
- **Company settings**: `language` on the company settings endpoints
- **Per-signing-request email language**: the `language` field on `POST /signing-requests`, `POST /signing-requests/create-and-send`, and `PATCH` / `PUT /signing-requests/{id}`
- **Signer terms**: the `{language}` path parameter on the company and workspace signer-terms endpoints (`PUT` / `DELETE`)
- **Email template defaults**: `GET /email-templates/defaults/{language}`
- **Organization seal statement**: `statement.language` on seal creation and update, returned as `statement_language` on the seal

Setting Catalan localizes the full signer experience: signer-facing emails, the signing ceremony UI, the consent statement recorded at terms acceptance, and date formatting. The completion certificate follows the workspace language, so set Catalan on the workspace for a Catalan certificate.

Validation errors for unsupported codes now list all 15 languages.

#### Migration notes

- The API change is additive. The existing 14 language codes are accepted and returned exactly as before, and requests that never send `ca` validate as before.
- Unless the signing URL carries a supported `lang` code, the signing page selects the first language in the signer's browser preference list that Firma.dev supports. A signer whose browser lists `ca` or a `ca-` regional tag ahead of every other supported language now sees the signing page in Catalan. Before v1.38.0, the signer saw the next supported language in the list or English. Signing links sent by email do not include a `lang` parameter, so this behavior also applies to existing signers. A supported `lang` code you add to an embedded signing URL still sets the language; any other value is ignored.
- The consent statement resolves in this order: workspace terms in the signer's language, company terms in the signer's language, workspace terms in the workspace's configured language, company terms in the company's configured language, then Firma.dev's default terms in the signer's language. If no Catalan terms or terms in the applicable configured language exist, a Catalan-first signer now accepts Firma.dev's default Catalan statement instead of custom terms in another browser language, such as Spanish. Terms in the workspace's or company's configured language still apply when available.
- Add Catalan signer terms at the company or workspace level to keep custom terms for these signers. Use `PUT /company/signer-terms/ca`, `PUT /workspace/{workspace_id}/signer-terms/ca`, or the dashboard terms settings.
...
**OpenAPI Spec:** [Download v1.37.0 spec](/api-reference/v01.37.00/openapi-v01.37.00.json) (unchanged in this release)

### CC recipients

The update endpoints now store recipients with `"designation": "CC"`, and the read endpoints now return them. Before this release they did neither.

#### Adding and updating CC recipients

- `PUT /templates/{id}` (`users[]`) and `PATCH /templates/{id}` (`user`) store CC recipients. Previously these returned `400` and `500`.
- `PUT /signing-requests/{id}` (`recipients[]`) and `PATCH /signing-requests/{id}` (`recipient`) store CC recipients. Previously `PUT` returned `200` and counted the CC in `summary.recipients_created` without storing it, and `PATCH` returned `500`.
- On `PUT /signing-requests/{id}`, a recipient that fails to save is now listed in `details.recipients` of the `400` response instead of being counted in `summary.recipients_created`.
- Existing CC recipients can be updated by `id` and removed through `deleted_users` / `deleted_recipients`, the same way as signers and approvers.
- `first_name` and `email` are required for a new CC recipient.
- `designation` is matched case-insensitively (`"CC"`, `"cc"`).

#### CC recipients in responses

CC recipients are returned with `"designation": "CC"`, after signers and approvers, by:

- `GET /signing-requests/{id}/users`
- `GET /templates/{id}/users`
- `GET /templates/{id}` and `GET /templates` (`recipients`)
- the `recipients` array of the `POST /signing-requests`, `POST /signing-requests/create-and-send`, and `PUT /signing-requests/{id}` responses

CC entries carry the same keys as the other entries on that endpoint. They never get a signing link, and keys that only apply to signing hold neutral values (for example `required_fields: []` and `ready_to_send: true`).

#### Template-based creation

- `POST /signing-requests` and `POST /signing-requests/create-and-send` with `template_id` now copy the template's CC recipients to the new signing request. A template CC is skipped when a CC with the same email (case-insensitive) is already on the request.
- A request-body recipient with `"designation": "CC"` is always added as a CC recipient. It is never matched to a template user by `template_user_id` or `order`.

#### Validation

- Changing an existing recipient between CC and Signer or Approver returns `400`. `PATCH /templates/{id}` and `PATCH /signing-requests/{id}` return the message `Cannot change designation between CC and Signer/Approver; delete the recipient and create it again`, `PUT /signing-requests/{id}` lists it in `details.recipients`, and `PUT /templates/{id}` returns the generic `Validation errors occurred`. Delete the recipient and create it again with the new designation.
- A field cannot be assigned to a CC recipient. On `PUT /signing-requests/{id}`, including for a CC created in the same request through `_temp_id`, the field error reads `Cannot assign fields to CC recipient.`; other field paths reject a CC id as an invalid recipient.
- Updating or deleting a recipient by `id` now only matches recipients that have not been deleted. An already-deleted recipient is treated as not found instead of being written to.

#### Migration notes

- No new endpoints, fields, or enum values, so the v1.37.0 spec still applies.
- Recipient lists can now include CC entries. If your code treats every entry as a signer, check `designation`.
- Signing requests created from a template that has CC recipients now include those CC recipients, and they receive the completed document. Remove the CC recipients from the template if you do not want this.
- If you relied on `PUT /signing-requests/{id}` returning `200` when a recipient failed to save, handle the `400` response.
...
**OpenAPI Spec:** [Download v1.37.0 spec](/api-reference/v01.37.00/openapi-v01.37.00.json)

### Organization Seals

Electronic seals applied automatically on behalf of a company, as participants in the signing order. Setup, scope rules, and certificate output are described in the [Organization Seals guide](/guides/organization-seals).

#### New endpoints

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/seals` | Create a seal (`kind`: `uploaded`, `typed`, or `drawn`; `scope`: `company` or `workspace` with `workspace_id`). Requires a protected API key for company scope |
| `GET` | `/seals` | List seals visible in the key's scope |
| `GET` | `/seals/{id}` | Get one seal |
| `PATCH` | `/seals/{id}` | Replace the image (new version), rename, update `display_name` / `signatory_title`, or set `is_default` |
| `DELETE` | `/seals/{id}` | Revoke; add `?stop_pending=true` to pause in-flight participants |
| `GET` | `/seals/{id}/image` | Canonical PNG; every read is logged |
| `GET` | `/seals/{id}/applications` | Signing requests the seal was applied to |
| `GET` | `/seals/{id}/access-log` | Lifecycle and image-read events |
| `DELETE` | `/seals/{id}/erase` | Erase a revoked lineage that no request or template references, 90 days after revocation |
| `DELETE` | `/signing-requests/{id}/seal-participants/{participant_id}` | Remove a seal participant and its fields from an unsent request |

#### Seal participants

Signing requests and templates accept a `seal_participants[]` array alongside `recipients[]` / `users[]` on `POST /signing-requests`, `POST /signing-requests/create-and-send`, `PATCH /signing-requests/{id}`, `POST /templates`, and `PATCH /templates/{id}`. Each entry takes `temp_id` (client id used to assign fields), `seal_id`, and `order`. Recipients and seals share one contiguous order space; order 1 applies at send.

Fields owned by a seal set `seal_participant_temp_id` instead of a recipient. Seal fields are server-authored (`read_only: 1`, no recipient) and client-provided values on them are ignored. Signing-request responses now include `seal_participants[]`.

#### Seal swap

`PATCH /signing-requests/{id}` accepts `seal_participant: { "id": "<participant-id>", "swap_to_seal_id": "<seal-uuid>" }` to move a paused participant on a sent request to a different in-scope seal. Requires a protected API key.

#### Webhook events

| Event | Trigger |
|---|---|
| `organization_seal.created` | Seal created |
| `organization_seal.updated` | Image replaced (new version), renamed, or default flag changed |
| `organization_seal.deleted` | Seal revoked |
| `organization_seal.erased` | Revoked lineage permanently erased |
| `signing_request.seal.applied` | Seal applied to a document |
| `signing_request.seal.paused` | Seal participant paused (revoked with stop pending, or integrity mismatch) |
| `signing_request.seal.swapped` | Paused participant swapped to a different seal |

#### Error codes

`SEALS_DISABLED`, `SEAL_NOT_FOUND`, `SEAL_SCOPE_FORBIDDEN`, `SEAL_IMAGE_INVALID`, `SEAL_ALREADY_REVOKED`, `SEAL_ERASE_NOT_ELIGIBLE`, `SEAL_MUTATION_NOT_ALLOWED`, `SEAL_ORDER_COLLISION`, `SEAL_PAUSED`, `SEAL_UNAVAILABLE`, and `SEAL_CREATION_DISABLED_EDGE` (reserved for the edge fallback host; the primary API host performs seal creation).

#### Migration notes

- Purely additive. `recipients[]`, `users[]`, and all existing endpoints are unchanged.
- No breaking changes.
...
**OpenAPI Spec:** [Download v1.36.0 spec](/api-reference/v01.36.00/openapi-v01.36.00.json)

### Anchor tags

- `add_white_background` on anchor tag definitions draws a white background across the full resolved field rectangle, independently of anchor-text removal.

### Error responses

- Error objects carry a machine-readable `code`.
- Validation failures also return `errors[]` with every error when multiple failures are reported together; the top-level error repeats the first item for backward compatibility.

### Create and send

- `POST /signing-requests/create-and-send` responses include `warnings[]`: optional non-blocking warnings, including unknown anchor-tag properties during the compatibility window and anchor-processing warnings.
...
**OpenAPI Spec:** [Download v1.35.0 spec](/api-reference/v01.35.00/openapi-v01.35.00.json)

### Cross-workspace template copy

New endpoint `POST /templates/{id}/copy` deep-copies a template to another workspace within the same company. All child data is copied with correct ID remapping:

- Template settings (expiration, signing order, email flags, completion page, etc.)
- Recipients (signers, approvers) with designation preserved
- Fields with `required_conditions` and `visibility_conditions` field references remapped
- CC recipients
- Custom field definitions
- Reminders with recipient references remapped
- PDF document (independently stored copy)

**Request body:**

- `workspace_id` (required, uuid) - target workspace to copy the template into
- `name` (optional, string, max 255) - name for the copy (defaults to original name with " (copy)" suffix)

**Authorization:** requires a company-level (protected) API key. Workspace-scoped keys receive `403 PROTECTED_KEY_REQUIRED`.

**New endpoint:**

- `POST /templates/{id}/copy` - copy template to another workspace
...
**OpenAPI Spec:** [Download v1.34.0 spec](/api-reference/v01.34.00/openapi-v01.34.00.json)

### Completion page customization

Four new nullable fields on signing requests, templates, workspace settings, and company settings control what signers see after completing their signature:

- `completion_title` (string, max 200 chars) - custom heading on the completion page
- `completion_message` (string, max 1000 chars) - custom body text
- `completion_redirect_url` (string, max 2000 chars, https only) - URL to redirect signers to after signing
- `completion_redirect_delay` (integer, 0-30) - seconds before redirect (defaults to 5 when a URL is set)

Settings cascade from company to workspace to template to signing request. Each level can override the parent, and each field resolves independently. Setting a field to `null` (or omitting it) inherits from the parent level; the built-in translated completion page is the final fallback.

**Affected endpoints:**

- `POST /signing-requests` - accepts completion fields
- `POST /signing-requests/create-and-send` - accepts completion fields
- `PUT /signing-requests/{id}` - accepts completion fields
- `PATCH /signing-requests/{id}` - accepts completion fields
- `GET /signing-requests/{id}` - returns completion fields
- `POST /templates` - accepts completion fields
- `PUT /templates/{id}` - accepts completion fields
- `PATCH /templates/{id}` - accepts completion fields
- `GET /templates/{id}` - returns completion fields
- `PUT /workspace/{id}/settings` - accepts completion fields
- `GET /workspace/{id}/settings` - returns completion fields
- `PUT /company/settings` - accepts completion fields
- `GET /company/settings` - returns completion fields

**Validation:**

- `completion_redirect_url` must use `https://`. For test-mode signing requests (`is_test: true`), `http://localhost` and `http://127.0.0.1` are also accepted.
- Redirect is time-limited: the auto-redirect only fires within 10 minutes of signing completion. After that, the custom title and message still display but no redirect occurs.

### Migration guide

No breaking changes. All new fields are nullable and default to `null`. Existing signing requests, templates, and settings are unaffected - the completion page renders its built-in translated content when no custom settings are configured.
...
[Download OpenAPI Spec](/api-reference/v01.33.00/openapi-v01.33.00.json)

### Five new languages

The supported language set grows from 9 to 14: Czech (`cs`), Swedish (`sv`), Dutch (`nl`), Romanian (`ro`), and Norwegian Bokmål (`nb`) join `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`.

The new codes are accepted at every language-bearing surface of the API:

- **Workspace settings** — `language` on `PUT /workspace/{workspace_id}/settings` (and returned by the corresponding GET)
- **Company settings** — `language` on the company settings endpoints
- **Per-signing-request email language** — the `language` field on `POST /signing-requests`, `POST /signing-requests/create-and-send`, and `PATCH` / `PUT /signing-requests/{id}`
- **Signer terms** — the `{language}` path parameter on the company and workspace signer-terms endpoints (`GET` / `PUT` / `DELETE`)
- **Email template defaults** — `GET /email-templates/defaults/{language}`

Setting one of the new languages localizes the full signer experience: signer-facing emails, the signing ceremony UI, the consent statement recorded at terms acceptance, date formatting, and the completion certificate (including correct rendering of Czech and Romanian diacritics).

Validation errors for unsupported codes now list all 14 languages.

### Migration Guide

No action required. The change is purely additive — the existing 9 language codes behave exactly as before, and workspaces or signing requests that never set a language are unaffected.
