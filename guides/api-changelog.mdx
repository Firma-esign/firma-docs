...
A new endpoint retrieves a download URL for a signing request's document. For completed signing requests, the URL points to the final signed PDF. For in-progress requests with partial download enabled, the URL points to a partially-signed document capturing the state after the most recent signer.
...
|-------|------|-------------|
| `status` | `string` | `finished` or `in_progress` — whether all signers have completed |
| `is_partial` | `boolean` | `true` when not all signers have completed |
...
| Sent, partial download disabled | `409 Conflict` — download unavailable |
| Sent, partial download enabled, document not generated yet | `503 Service Unavailable` with `Retry-After` header |
| Sent, partial download enabled, document available | `200` with `is_partial: true` |
| Completed (all signers finished) | `200` with `is_partial: false` |

**Partial downloads** require the signing request to have `allow_partial_download` enabled in settings. When partial generation is in progress, a `503` response includes a `Retry-After` header indicating when to retry.
...
- The new endpoint is purely opt-in. No existing behavior changes.
- To use partial downloads, enable `allow_partial_download` in signing request settings, then poll `GET /signing-requests/{id}/download` — handle `503` with `Retry-After` until the partial document is ready.
