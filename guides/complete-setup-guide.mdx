...
- **Pay-as-you-go** pricing at €0.049/envelope (or about 5 cents USD) - no subscriptions, no hidden fees
...
3. Firma provides **one unified environment** with envelopes priced so low (just **€0.049 each**) that testing and iterating costs virtually nothing.
...
Both keys are returned when you create a workspace (`api_key` = live, `test_api_key` = test) and from the Get Workspace endpoint, and both appear in your dashboard. See [Authentication](/guides/authentication) for how test mode works.
...
Firma uses a **credit-based** pricing model. Every envelope (a document sent for signing) costs just **€0.049** (or about 5 cents USD). There are no subscriptions, no minimums, and no surprise fees.
...
[Start now](https://www.firma.dev) - at €0.049 per envelope, getting started costs virtually nothing.