...
The `api_key` is the workspace's live key and `test_api_key` is its test key. See [Authentication](/guides/authentication) for how test mode works.
...
- [Send signing requests](/guides/sending-signing-request) within workspaces (100 req/min)
- [Set up webhooks](/guides/webhooks) to track workspace activity (60 req/min)
- [JWT authentication](/guides/embeddable-template-editor#jwt-authentication) for embedded workflows (120 req/min)