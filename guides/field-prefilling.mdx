...
| :--- | :--- | :--- | :--- |
| `variable_name` | Top-level field property (request and response) | The recipient-attribute key for this field. When set to a known attribute — `email`, `first_name`, `last_name`, `full_name`, `phone_number`, `company`, `title`, `street_address`, `city`, `state_province`, `postal_code`, `country` — the field is auto-populated from the recipient's data at send time, and the system requires that attribute to be present on the recipient before allowing send. Also used as a **template merge key** (fallback after `template_field_id`) and as an **API accessor** to look up fields in `GET /signing-requests/{id}/fields` responses. For fields that don't map to a recipient attribute, use any string as a label. | Yes — when it matches a known recipient attribute |
| `read_only_value` | Top-level field property (request) | Static text you supply at field-creation time. **Cannot be combined with prefilled fields** — the API returns 400 if both are set. When a template already carries a prefill rule, `read_only_value` on the per-request field overrides the template's rule during merge. | Yes — always wins when present |
| `format_rules.prefilledData` | Nested inside `format_rules` on the field | The explicit recipient-attribute key, stored in `format_rules`. For prefilled fields, this is typically set to the same value as `variable_name` — they work together. Set this when creating fields directly via the API; for template-based fields, it's set automatically from the field type. Recognized keys: `email`, `first_name`, `last_name`, `full_name`, `phone_number`, `company`, `title`, `street_address`, `city`, `state_province`, `postal_code`, `country`, or a `custom_fields` key. | Yes — when `read_only_value` is absent |
