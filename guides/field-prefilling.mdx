...
| :--- | :--- | :--- | :--- |
| `variable_name` | Top-level field property (request and response) | A label you assign to a field. Used as a fallback matcher when merging fields into a signing request created from a template. **Never reads from or writes to recipient data.** | No |
