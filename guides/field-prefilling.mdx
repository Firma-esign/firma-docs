...
| :--- | :--- | :--- | :--- |
| `variable_name` | Top-level field property (request and response) | A developer-defined key you assign to a field. Serves three purposes: (1) **API accessor** — use it to look up a specific field's value in `GET /signing-requests/{id}/fields` responses, (2) **template merge key** — when creating from a template, fields are matched by `variable_name` as a fallback after `template_field_id`, and (3) **send-readiness validation** — if `variable_name` matches a known recipient attribute (`email`, `first_name`, `phone_number`, `company`, `title`, etc.), the system requires that attribute to be present on the recipient before allowing send. It does **not** control the prefilled value itself — that's `format_rules.prefilledData`. | No |
| `read_only_value` | Top-level field property (request) | Static text you supply at field-creation time. **Cannot be combined with `format_rules.prefilledData` on the same field** — the API returns 400 if both are set. When a template already carries a `prefilledData` rule, `read_only_value` on the per-request field overrides the template's rule during merge. | Yes — always wins when present |
| `format_rules.prefilledData` | Nested inside `format_rules` on the field | The recipient-attribute key to auto-populate from (`email`, `full_name`, `phone_number`, `company`, etc., or a `custom_fields` key on the recipient). Resolved at send time from the assigned recipient's data. **Cannot be combined with `read_only_value` on the same field** — the API returns 400. | Yes — when `read_only_value` is absent |
