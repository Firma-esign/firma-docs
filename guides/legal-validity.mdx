...
eIDAS also defines electronic seals (Articles 35-36), the equivalent of electronic signatures but for legal persons (companies) rather than natural persons. The three-tier structure mirrors signatures:

| Tier | eIDAS basis | What it takes |
|---|---|---|
| Electronic Seal | Art. 3(25) | Data in electronic form attached to other data, used by a legal person to ensure origin and integrity |
| Advanced Electronic Seal | Art. 3(26), Art. 36 | Must be uniquely linked to the creator (the legal person), capable of identifying that person, created using data under the creator's control, and linked to the data so any later change is detectable |
| Qualified Electronic Seal | Art. 3(27) | An advanced seal created on a Qualified Seal Creation Device, using a Qualified Certificate issued by a QTSP |

Firma's organization seal is designed to meet the **Advanced Electronic Seal** bar:

- **Uniquely linked to the company** - each seal is bound to the company that created it, with a recorded attestation from an authorized representative
- **Identifies the company** - the company name, seal display name, and signatory title appear on the certificate of completion
- **Created under the company's control** - only company owners or holders of the company's protected API key can create, replace, or revoke a seal; the stored image is hash-verified before every application
- **Linked to the document** - the seal is applied as part of the PAdES digital seal on the completed PDF, so any later change to the document is detectable
...
Organization seals applied by Firma are advanced electronic seals, not qualified ones; see the next section. Setup and API details are in the [Organization Seals guide](/guides/organization-seals).
...
### Organization seals under ESIGN and UETA

US law does not separate seals from signatures. ESIGN (15 U.S.C. § 7006(5)) defines an electronic signature as any symbol or process adopted by a person with intent to sign, and a company is a person. ESIGN (§ 7006(3)) and UETA (§ 2(6), § 14) also recognise an electronic agent acting for a person, and UETA § 9 attributes its acts to that person.

An organization seal fits that model: by creating the seal and accepting the authority statement, the company adopts the process, and each application is the act of an agent it authorized. In the US the seal can therefore serve as the company's electronic signature and, on company documents, stand in for the signature of an employee acting on the company's behalf, such as a countersigning officer. The certificate still records it as "Applied automatically" with the company, the attester, and the timestamp, which is the attribution evidence UETA § 9 asks for. Whether that satisfies a statute or clause that requires a personal signature is a question for your counsel.
