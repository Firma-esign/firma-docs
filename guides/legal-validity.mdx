...
- **Tamper-evident digital seal** — for workspaces with PAdES sealing enabled, Firma applies a PAdES digital seal (the ETSI standard signature format for PDFs) to the final document on completion. The seal embeds the signing certificate, a trusted timestamp, and revocation information directly in the file, so it can be verified independently of Firma's own servers.
...
- **No SOC 2 or ISO 27001 certification yet.** If that changes, this page will be updated.
