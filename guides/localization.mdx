...
title: "Localization"
icon: "globe"
description: "Firma supports 15 languages out of the box with automatic language detection, per-user persistence, and embed-friendly overrides."
---

Firma ships with complete localization in 15 languages, making it ready for teams across Europe and Latin America from day one.
...
On a user's first visit, Firma automatically matches the language to their browser or system language setting. If the detected language isn't one of the 15 supported languages, Firma defaults to English.
...
You can combine `lang` with other URL parameters like `zoom` (initial zoom level):

```
https://app.firma.dev/signing/{signing_request_user_id}?lang=es&zoom=1.0
```
...
See [Embeddable signing](/guides/embeddable-signing#url-parameters) for all available URL parameters.

### Supported values

Use the two-letter language codes from the table above: `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro`, `nb`, `ca`.
...
- [**Embeddable signing request editor**](/guides/embeddable-signing-request-editor) — The signing request configuration interface supports all 15 languages
