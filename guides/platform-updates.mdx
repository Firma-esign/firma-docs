...
- When the signing URL has no `lang` parameter, the signing ceremony and the consent statement recorded at terms acceptance follow the signer's own browser language, detected automatically; a `lang` parameter on the signing URL sets the language instead
