...
description: "Follow every new feature, improvement, and change shipped to the Firma e-signature platform, with dates, highlights, and migration notes for each release."
icon: "sparkles"
---
...
See the [API changelog](api-changelog#v1-26-0) for the `signing_button_label_overrides` field on workspace settings.
...
Available in the template editor, signing request editor, embedded editors, and the full REST API. See the [API changelog](api-changelog#v01-10-00) for schema details.
...
You can now require hand-drawn signatures via the API by setting `hand_drawn_only: true` in your signing request settings. When enabled, signers cannot use typed or font-based signatures and must draw their signature directly. See the [API changelog](api-changelog#v01-07-00) for details.
