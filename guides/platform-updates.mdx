...
Firma now speaks 15 languages. Catalan (`ca`) joins the existing fourteen across the entire platform: the dashboard, the signer-facing signing experience, all signer emails, signer consent terms, date formatting, completion certificates, and the organization-seal statement and its lifecycle emails.

- Set the language per workspace, per company, or per individual signing request, via the dashboard or the API. This drives invitation and reminder emails and date formatting. The completion certificate follows the workspace language
- Unless the signing URL carries a supported `lang` code, the signing ceremony and the consent statement recorded at terms acceptance follow the signer's own browser language, detected automatically; a supported `lang` code on the signing URL sets the language instead
- Certificates render Catalan diacritics and the geminated L (l·l) correctly with embedded fonts
- Browsers reporting a Catalan regional or dialect tag (`ca-ES`, `ca-AD`, `ca-FR`, `ca-IT`, `ca-valencia`) are detected as `ca` automatically

See the [API changelog](/guides/api-changelog#v1-38-0) for the widened language enums.
...
Automate your company's countersignature. An organization seal places a company stamp, display name, and signatory title on the completed PDF as a participant in the signing order, and the certificate of completion records it as "Applied automatically".

- **Company and workspace scopes**: manage seals under **Settings > Organization Seals** (company) or a workspace's **Settings > Organization Seals**, with one default per scope and sender override
- **Three creation modes**: upload a PNG, type the company name, or draw a mark, each with a recorded authority attestation
- **Placement anywhere in the order**: add a seal from the **Template Users** or **Signers** panel; the top slot applies at send, any other slot applies once the participants before it have finished
- **Revoke and swap**: revoke prospectively, optionally stop pending applications, and swap a paused seal for another from the request view
- **API and webhooks**: `seal_participants[]` on templates and signing requests, `/seals` management endpoints, and seven webhook events

See the [Organization Seals guide](/guides/organization-seals) for setup and the [API changelog](/guides/api-changelog#v1-37-0) for endpoint details.
...
You can now download PDF invoices for your credit purchases directly from the transaction history. Go to **Account > Credits & Billing** and click **View** next to any purchase to open the invoice in a new tab.

- Works for all past and future credit purchases
- Invoices are fetched on demand from Paddle, so they always reflect the latest billing details including VAT corrections
- Available for both one-time and auto-recharge purchases
...
You can now customize what signers see after they finish signing. Set a custom title, message, redirect URL, and redirect delay at the workspace, template, or individual signing request level.

- **Custom title and message** replace the default "Signature Complete!" heading and description with your own branded text
- **Auto-redirect** sends signers to your app or a confirmation page after a configurable delay (0-30 seconds)
- **Cascading settings** let you set company-wide defaults and override them per workspace, template, or signing request
- Configure completion settings in **Workspace Settings > Completion Page** or in the **template editor's Document Settings** tab
- Available via the API on all signing request, template, workspace, and company settings endpoints

See the [API changelog](/guides/api-changelog#v1340) for field details and validation rules.

---

## August 3, 2026

### Five New Languages: Czech, Swedish, Dutch, Romanian, Norwegian

Firma now speaks 14 languages. Czech (`cs`), Swedish (`sv`), Dutch (`nl`), Romanian (`ro`), and Norwegian Bokmål (`nb`) join the existing nine across the entire platform: the dashboard, the signer-facing signing experience, all signer emails, signer consent terms, date formatting, and completion certificates.

- Set the language per workspace, per company, or per individual signing request — via the dashboard or the API
- The full signer journey is localized: invitation and reminder emails, the signing ceremony, the consent statement recorded at terms acceptance, and the completion certificate
- Certificates render Czech and Romanian diacritics correctly with embedded fonts
- Browsers reporting Norwegian as `no` or `nn` are detected as `nb` automatically

See the [API changelog](/guides/api-changelog#v1330) for the widened language enums.
