...
**Quick answer:** Firma's infrastructure runs entirely in the EU, with encryption in transit and at rest, row-level access control on every database table, and a signing process backed by a full audit trail. Firma does **not** currently hold SOC 2 or ISO 27001 certification, and does not run a formal bug bounty program. Security reports are welcomed and acted on. Details below.
...
| Digital signature sealing | DSS (Digital Signature Service) | Applies PAdES cryptographic seals to completed documents (enabled per workspace — not yet applied to all documents by default) |
...
Firma is direct about where it stands today:

- **No SOC 2 certification.** Firma does not currently hold a SOC 2 Type I or Type II report.
- **No ISO 27001 certification.** Firma is not currently ISO 27001 certified.
...
If your procurement process requires one of these as a hard gate, Firma doesn't yet meet that bar — we'd rather tell you that directly than have it surface later in a security questionnaire. This page will be updated if that status changes.

<Note>
  For the legal-validity side of compliance — eIDAS signature tiers, ESIGN/UETA, and country-specific rules — see [Legal Validity & eIDAS Compliance](/guides/legal-validity). That page also covers the current absence of SOC 2/ISO 27001 certification in the context of signature legal weight.
</Note>
...
- [Legal Validity & eIDAS Compliance](/guides/legal-validity) — signature tiers, admissibility, and the same SOC 2/ISO 27001 disclosure in a legal context
