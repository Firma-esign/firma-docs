...
| --- | --- | --- |
| Per-webhook `enabled` | An individual webhook subscription | Set on the webhook itself (`PATCH /webhooks/{id}`) |
...
<Note>
  **Rate limit**: Webhook read operations (GET) support **200 requests per minute per API key**. Webhook write operations (create, update, delete) are limited to **60 requests per minute**.
</Note>
