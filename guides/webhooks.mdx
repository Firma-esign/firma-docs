...
### Organization Seal Events

- `organization_seal.created` - New seal created
- `organization_seal.updated` - Seal image replaced (new version), renamed, or default flag changed
- `organization_seal.deleted` - Seal revoked
- `organization_seal.erased` - Revoked seal lineage permanently erased

### Signing Request Seal Events

- `signing_request.seal.applied` - Seal applied to a document
- `signing_request.seal.paused` - Seal participant paused (revoked seal with stop pending, or integrity mismatch)
- `signing_request.seal.swapped` - Paused seal participant swapped to a different seal
...
| Switch | Scope | Where it's set |
| --- | --- | --- |
| Per-webhook `enabled` | An individual webhook subscription | Set on the webhook itself (`PUT /webhooks/{id}`) |
| Master switch | Company or workspace | Company: dashboard **Settings > Webhooks > Enable Webhooks** toggle only. Workspace: dashboard, or `PATCH /workspaces/{id}` with `{"webhook_enabled": true}` |
...
3. After 7 days, only `X-Firma-Signature` is sent
...
**Webhooks configured but events don't fire**

This is the most common cause of "my webhook isn't working" reports. Per-webhook settings can be entirely correct while the account-level master switch is off, silently skipping every event.

- Confirm the [master switch](#two-levels-of-enablement) is on for the right scope: company-level webhooks need the company's switch on, workspace-level webhooks need that workspace's switch on.
