...
<Note>
  All API examples on this page pass your API key directly in the `Authorization` header. The `Bearer` prefix is optional - both `Authorization: YOUR_API_KEY` and `Authorization: Bearer YOUR_API_KEY` work.
</Note>
...
| `color_accent` | Secondary accent color | `#34eeff` |
| `color_accent_fg` | Text color on accent elements | `#000000` |
| `color_canvas` | Document canvas/viewer background | `#0f1419` |
| `color_muted` | Muted/subtle background areas | `#22222a` |
| `color_muted_fg` | Text color on muted areas | `#b8b8b8` |
...
| `show_qr_code` | Include a QR code linking to the signing page in emails | `true` |
...
`require_terms_acceptance` follows the same cascade as other branding settings: **workspace setting → company setting → Firma default**. The Firma default is `true` (enabled), so terms acceptance is ON for every document unless you explicitly disable it at the company or workspace level.
...
3. **Workspace terms** for the workspace's configured language
4. **Company terms** for the company's configured language
5. **Firma default terms** (lowest priority)

<Note>
  Terms acceptance is tracked per signer, per signing request - not per signer email address. A signer who accepts terms on one document must accept them again on the next document; there is no cross-document "accept once" state.
</Note>
...
`require_terms_acceptance` can be turned off for your entire account or for a single workspace.

**Disable for your entire account** (all workspaces, unless a workspace overrides it):
...
**Disable for a single workspace** (overrides the company setting for that workspace only):
...
Set `require_terms_acceptance` to `null` at the workspace level to clear the override and inherit the company setting again.

<Note>
  If the workspace dashboard toggle for this setting shows a permission error, only company or workspace admins can change this setting from the dashboard UI. Team members without admin access should ask an admin to update it, or use the API directly with your API key, which is not subject to the dashboard's admin restriction.
</Note>

Supported languages: `en`, `es`, `it`, `pt`, `fr`, `de`, `el`, `ru`, `pl`, `cs`, `sv`, `nl`, `ro`, `nb`, `ca`.
...
**Validation:**
- Each label must be a string - values over 200 characters are silently truncated
- Language keys are strings (max 10 characters, not validated against the supported-language list)
...
See the [Email Domains API Reference](/api-reference/v01.27.00/email-domains/list-company-domains) for complete endpoint documentation, or [Custom Domains](/guides/custom-domains) for DNS record details, Resend conflict resolution, and verification state troubleshooting.
...
**Sender name** is the workspace name (falling back to the company name if the workspace has no name set).

For example, if your workspace is named "Acme Legal" and you set `email_local_part` to `noreply` with a verified domain `sign.acmecorp.com`, emails will be sent from:
...
[Embeddable Signing Guide](/guides/embeddable-signing) - Full implementation details

### Embeddable template editor

Let users create and edit templates within your app using JWT authentication. The editor loads via a script tag and mounts into a container element via Shadow DOM - not an iframe:
...
[Embeddable Template Editor Guide](/guides/embeddable-template-editor) - JWT generation, events, and full implementation

### Embeddable signing request editor

Same pattern - script tag + constructor, not an iframe:
...
[Embeddable Signing Request Editor Guide](/guides/embeddable-signing-request-editor) - full implementation guide
...
- Token expiration is fixed server-side: 24 hours for template editor tokens, 7 days for signing-request tokens - not configurable by the caller
...
- [Creating Workspaces](/guides/creating-workspaces) - Set up isolated environments for customers
- [Webhooks](/guides/webhooks) - Track signing events in real-time
