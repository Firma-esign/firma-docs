...
2. **Custom email templates** — Personalize the content and branding of signing notification emails
3. **Disable Firma emails** — Turn off automatic emails per signing request and send your own
4. **Embedded experiences** — Embed signing and template editors directly in your app
...
📘 See the [Email Domains API Reference](/api-reference/v01.09.00/email-domains/list-company-domains) for complete endpoint documentation.
...
Templates support HTML bodies with dynamic placeholders. Use the `GET /email-templates/placeholders` endpoint to see all available variables.
...
This lets you set a company-wide branded template and override it for specific workspaces when needed.

<Warning>
  A warning is returned if a `signing_invite` template body does not contain the `{{signing_link}}` variable, since recipients need a link to access the signing flow.
</Warning>

📘 See the [Email Templates API Reference](/api-reference/v01.09.00/email-templates/list-workspace-email-templates) for complete endpoint documentation.
...
- ✅ Use [custom email templates](#custom-email-templates) to match your brand voice and style
- ✅ Set company-level email templates as a baseline, then override per workspace where needed
