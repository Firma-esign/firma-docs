...
description: "Una timeline di nuove funzionalità, miglioramenti e modifiche alla piattaforma Firma"
icon: "sparkles"
---
...
See the [API changelog](api-changelog#v1-26-0) for the `signing_button_label_overrides` field on impostazioni del workspace.
...
- Each color è opzionale: unset values inherit workspace → company → Firma default, so existing editors render unchanged
- Asymmetric-contrast warnings flag color pairs that may be hard to read

See the [API changelog](api-changelog#v1-25-0) for the new `color_*` fields and the 11-key resolved palette.
...
See the [API changelog](api-changelog#v1-23-0) for the Signer Terms and company settings endpoints.
...
Signer sessions on OTP-protected richiesta di firmas now have a firm time limit. After a signer verifies their email with a one-time code, access stays valid through normal activity but can no longer stay open indefinitely. When the limit is reached, the signer re-verifies with a new code to continue, no action needed from senders.

- Applies to richiesta di firmas with OTP verification enabled
...
See the [API changelog](api-changelog#v1-21-0) for schema details.
...
- Available as a toggle in company settings, impostazioni del workspace, editor di template, and richiesta di firma editor
- Cascading setting: richiesta di firma overrides workspace, which overrides company
- Set at the company level to apply across all workspaces, or override per workspace/template/richiesta di firma
- Also available via the API as `disable_guided_navigation` in settings

See the [API changelog](api-changelog#v1-20-0) for schema details.
...
See the [API changelog](api-changelog#v1-19-0) for the new logo management endpoints.
...
The signing view now guides signers through documents field by field. Quando un signer agrees to terms and the document loads, the first required field is automatically highlighted and scrolled into view.
...
- Configure which identity fields are editable per richiesta di firma or template
...
See the [API changelog](api-changelog#v1-18-0) for schema details.
...
- Enable via the "Allow signer to edit this value" checkbox in the template or richiesta di firma editor
...
You can now set an explicit `value` on fields when creating richiesta di firmas via the API. Questo ti consente di pre-populate any field with a specific value, independent of prefilled data or read-only settings. When both an explicit value and prefilled data are set, the explicit value takes precedence.
...
See the [API changelog](api-changelog#v1-18-0) for the `prefilled_editable` property and field value details.
...
You can now download a snapshot PDF of richiesta di firmas that are still in progress. From the richiesta di firma detail panel, click the download button to get a PDF showing all fields completed so far, with an audit trail page listing each signer's status.

- Available for in-progress, cancelled, declined, and expired richiesta di firmas
- Only completed signers' fields are rendered; pending signers' fields are omitted
- An optional "IN PROGRESS" watermark can be enabled per workspace in impostazioni del workspace
- The PDF includes a per-signer status table and evidentiary framing

See the [API changelog](api-changelog#v1-16-0) for the corresponding `GET /signing-requests/{id}/download` endpoint.
...
A new toggle in impostazioni del workspace controls whether partial PDF downloads include a diagonal "IN PROGRESS — NOT YET EXECUTED" watermark. The setting defaults to disabled and can be overridden per workspace.
...
Workspaces can now have their own webhook endpoints, independent from a livello di azienda webhooks. Each workspace gets its own signing secret and can subscribe to the same events as company webhooks.

- **Independent webhook configuration** — Create, test, and manage webhook endpoints scoped to a specific workspace from the impostazioni del workspace tab
- **Workspace signing secret** — Each workspace generates and rotates its own secret di firma webhook with a 7-day grace period, separate from the company secret
- **Ignore company webhooks** — Toggle to prevent a livello di azienda webhooks from firing for events in a workspace, allowing full webhook isolation per workspace
- **Event log** — View delivery history and retry failed events for workspace webhooks directly in impostazioni del workspace

See the [API changelog](api-changelog#v01-15-00) for endpoint and schema details.
...
You can now assign three distinct roles to recipients in richiesta di firmas and templates:

- **Signer** — Signs the document with signature, initial, and signing date fields (existing behavior)
- **Approver** — Approves the document using dedicated approval fields: approval stamp, approval checkmark, and approval date. Approvers must complete all assigned approval fields before the richiesta di firma can finish.
...
The template and richiesta di firma editors now show separate sections for active participants (Signers and Approvers) and CC recipients. Designation is selected when adding a recipient, and field compatibility is enforced automatically — signer-only fields (signature, initial, signing date) are only available for Signers, and approval fields are only available for Approvers.

See the [API changelog](api-changelog#v01-14-00) for schema details.

### Stamp Fields

New stamp field type for placing image stamps or company seals on documents. Stamps support two modes:

- **Creator-configured**: Set the field as read-only, upload a PNG image in the template or richiesta di firma editor, and it appears as a fixed stamp at signing time
...
See the [API changelog](api-changelog#v01-13-00) for field type schema details.
...
Firma now supports Russian and Polish across the platform. Puoi impostare either language at the company, workspace, or user level. Signing flow emails, the signing interface, and the main application UI are all available in both languages.
...
API responses for template and richiesta di firma fields now include a `variable_defined_name` property — the human-readable name you defined when creating a custom field. Puoi anche use `variable_defined_name` to target fields when creating richiesta di firmas from templates. See the [API changelog](api-changelog#v01-12-01) for details.
...
You can now connect Firma to AI assistants like Claude using the Model Context Protocol (MCP). Una volta connesso, you can manage templates, create and send richiesta di firmas, configure workspaces, and more through natural language conversations.

- Works with Claude Code, Claude Desktop, and Claude.ai
- Authenticate with your existing API key or via OAuth sign-in
- Full access to workspaces, templates, richiesta di firmas, webhooks, and company settings
- Switch between workspaces during a conversation
- Each tool call maps to one API request, subject to standard rate limits

See the [MCP Integration guide](mcp) for setup instructions and the [API changelog](api-changelog#v01-12-00) for details.
...
A new `file` field type lets you collect file attachments from signers. Add a file upload field to any template or richiesta di firma, and signers can upload documents directly during the signing flow.
...
- Upload limit warnings shown in the editor di template

See the [API changelog](api-changelog#v01-11-00) for the `FileFormatRules` schema and field configuration details.

### Anchor Tags

Automatically place fields on documents using text markers. Embed anchor strings like `{{SIGN_HERE}}` or `{{DATE}}` in your PDFs, and Firma will find them and convert them into positioned fields during richiesta di firma creation. The anchor text is removed from the document after processing.

- Up to 100 anchor tags per richiesta di firma
...
See the [API changelog](api-changelog#v01-11-00) for the full `AnchorTag` schema and properties.
...
Fields on documents can now be conditionally visible and conditionally required based on what a signer enters in other fields. Ad esempio, a "Spouse Name" field can appear only when a signer checks "Married," or an address block becomes required only when "Ship to different address" is selected. Conditions use a flexible nested logic system (AND/OR groups) and evaluate in real-time as signers fill out the document. All conditions are enforced server-side on submission, so they can't be bypassed.

Available in the editor di template, richiesta di firma editor, embedded editors, and the full REST API. See the [API changelog](api-changelog#v01-10-00) for schema details.

### DOCX Document Support

Templates and richiesta di firmas now accept simple .docx files in addition to PDFs. Documents are automatically converted to PDF on upload — no extra steps needed. This works across the UI editors and the API (the document field accepts base64-encoded DOCX or PDF).
...
- Enable at the workspace or individual richiesta di firma level
...
See the [API changelog](api-changelog#v01-09-00) for the `require_otp_verification` setting details.
...
- Works in both the main editor di template and the embedded editor
- 60MB file size limit

See the [API changelog](api-changelog#v01-09-00) for the `POST /templates/{id}/replace-document` endpoint.

### Custom Email Templates

Customize the emails Firma invia on your behalf. Edit the subject and body for each notification type with dynamic variable support.
...
- Language cascades: richiesta di firma \> workspace \> company
...
You can now require hand-drawn signatures via the API by setting `hand_drawn_only: true` in your richiesta di firma settings. When enabled, signers cannot use typed or font-based signatures and must draw their signature directly. See the [API changelog](api-changelog#v01-07-00) for details.
...
These options are available on both `FirmaTemplateEditor` and `FirmaSigningRequestEditor`. See the [embeddable editor di template](embeddable-template-editor) and [embeddable richiesta di firma editor](embeddable-signing-request-editor) guides for full details.
