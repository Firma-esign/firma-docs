---
title: "Invio di una richiesta di firma"
description: "Come creare e inviare richieste di firma ai destinatari."
icon: "paper-plane"
---

# Sending a richiesta di firma

Questa guida covers creating a richiesta di firma, attaching a template or document, and inviting recipients to sign.

## Steps

1. Create or select a template
2. Create a richiesta di firma referencing the template
...
## Create a richiesta di firma from a template (API)
...
## Create a richiesta di firma (server example) — Node (fetch)
...
## Create a richiesta di firma (server example) — Python (requests)
...
<Warning>
  **Critical**: All field position coordinates (x, y, width, height) must be **percentages (0-100)** relative to page dimensions, not pixels. The `page_number` field è obbligatorio.
</Warning>

When creating a richiesta di firma directly (POST `/signing-requests`) or updating one, you can add form fields:
...
## Updating richiesta di firmas

Before a richiesta di firma is sent, you can update its details using the API. The API provides two methods:

<Warning>
  **Cannot update after sending**: Once a richiesta di firma is sent, it cannot be modified. Updates only work for requests with status `not_sent`.
</Warning>
...
| Scenario                                  | Use   | Reason                                                                         |
| ----------------------------------------- | ----- | ------------------------------------------------------------------------------ |
| Complete richiesta di firma rebuild          | PUT   | Full replacement with all sections (properties, recipients, fields, reminders) |
...
| Update custom fields                      | PATCH | Preserves core richiesta di firma                                                 |
| Replace all recipients                    | PUT   | Clean slate approach                                                           |

<Warning>
  **Important**: Both update methods only work **before** the richiesta di firma is sent. Once sent, the richiesta di firma becomes immutable to prevent tampering with active signature workflows.
</Warning>

**Best practices**:

- Update richiesta di firmas before calling `/send`
...
Once you have a richiesta di firma ID (and have made any necessary updates), call POST `/signing-requests/{signing_request_id}/send` to send emails to all recipients.
...
The public signing UI è disponibile at the pattern:
...
### Esempio — fetch signing details and render iframe

````js
### Getting the signing URL

- The `signing_request_user_id` is returned in the `recipients` array after creation
- Alternatively, fetch richiesta di firma details via GET `/signing-requests/{id}` to retrieve per-recipient signing links

### Esempio — fetch signing details and render iframe

```js
// fetch richiesta di firma
...
- [Embed the editor di template](embeddable-template-editor) with JWT authentication (120 req/min)
- [Configure impostazioni del workspace](workspace-settings) for custom email templates (100-200 req/min)
- [Embeddable editor di template](embeddable-template-editor) with JWT authentication for secure embedded workflows
