...
title: "Webhook"
icon: "webhook"
description: "Implementa integrazioni webhook sicure per ricevere notifiche di eventi in tempo reale da Firma."
---


Firma invia webhook events to notify your application about signing lifecycle changes, document completions, and workspace activities. Webhooks enable real-time integrations without polling.

## Common use cases

- Send internal notifications when documents are signed
- Update your database when richiesta di firmas are completed
- Trigger downstream workflows (invoicing, provisioning, etc.)
- Track richiesta di firma status changes in real-time

## Tipi di eventi

Firma invia the following event types:

### Signing Request Events

- `signing_request.created` - New richiesta di firma created
...
- `signing_request.recipient.added` - Recipient added to richiesta di firma
...
- `template.used` - Template used to create a richiesta di firma
...
## Sicurezza: Signature verification (Required)
...
Firma usa a timestamped signature header:
...
When you rotate your secret di firma webhook:

1. Firma generates a new secret
2. For 24 hours, Firma invia **both** signatures:
...
<Note>
  **Rate limit**: GET webhook requests support **60 requests al minuto per API key**.
</Note>
