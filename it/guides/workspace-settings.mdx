...
icon: "briefcase"
description: "Configura le impostazioni a livello di workspace, inclusi template email, informazioni del team e preferenze di fuso orario."
---


Workspace settings allow you to customize email templates, team contact information, and timezone preferences at the workspace level. These settings apply to all richiesta di firmas and templates within the workspace.

## Use cases

- **Custom email branding**: Personalize email headers and body text for richiesta di firma invitations
...
## Get impostazioni del workspace

Retrieve current impostazioni del workspace including email templates, team email, and timezone configuration.
...
## Update impostazioni del workspace

Update impostazioni del workspace. You can update one or more fields — only provided fields will be updated.
...
Returns the updated impostazioni del workspace:
...
app.get('/api/workspace/:workspaceId/settings', async (req, res) => {
  ...
  try {
    ...
    console.error('Failed to get impostazioni del workspace:', error)
...
app.put('/api/workspace/:workspaceId/settings', express.json(), async (req, res) => {
  ...
  try {
    ...
    console.error('Failed to update impostazioni del workspace:', error)
...
```
X-RateLimit-Limit: 200          # Max requests al minuto
...
- Test with a new richiesta di firma (not existing draft)
- Check workspace ID matches richiesta di firma
...
- [List Workspaces](../api-reference/v01.15.00/workspaces/list-workspaces) — Get all workspaces (100 req/min)
- [Create Workspace](../api-reference/v01.15.00/workspaces/create-a-new-workspace) — Create new workspace (100 req/min)
- [Update Workspace](../api-reference/v01.15.00/workspaces/update-a-workspace) — Update workspace details (100 req/min)
...
- [Update Workspace Settings](../api-reference/v01.15.00/workspace-settings/update-workspace-settings) — Update email templates and preferences (100 req/min)

### Related Endpoints

- [Generate JWT Token for Templates](../api-reference/v01.15.00/jwt-management/generate-jwt-token-for-embedding-templates) — For embedded editor di template (120 req/min)
- [Create Template](../api-reference/v01.15.00/templates/create-template) — Create templates per workspace (100 req/min)
- [Create Signing Request](../api-reference/v01.15.00/signing-requests/create-signing-request) — Send documents with workspace branding (100 req/min)
...
- [Send richiesta di firmas](sending-signing-request) with customized emails
- [Set up webhooks](webhooks) to track workspace activity
- [Embeddable editor di template](embeddable-template-editor) with JWT authentication for embedded integrations