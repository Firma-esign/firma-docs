---
description: "Este guia abrangente mostra como construir e lançar um fluxo completo de assinatura eletrônica dentro da sua própria aplicação usando Firma.dev. Cobriremos tudo, desde a configuração da conta até assinatura incorporada, gerenciamento de workspaces, templates e faturamento baseado em créditos."
icon: "flag"
...
Each workspace has two API keys: a **live** key and a **test** key. While you build your integration, use the **test** key — requests authenticated with it don't consume credits and produce test-flagged, watermarked documents, so you can iterate freely. Switch to the live key when you're ready for production.

Both keys are returned when you create a workspace (`api_key` = live, `test_api_key` = test) and from the Get Workspace endpoint, and both appear in your dashboard. See [Authentication](authentication) for how test mode works.
...
The `document` field accepts a base64-encoded PDF or DOCX file. The response returns the template ID, which you can then use to add recipients and fields via the template editor or API.
...
You can also create signing requests directly from a document (base64-encoded PDF or DOCX) instead of a template by passing `document` instead of `template_id`.
...
You can periodically poll the company endpoint to keep an eye on usage, or integrate it into internal monitoring and dashboards.
