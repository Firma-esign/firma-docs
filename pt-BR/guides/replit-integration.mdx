---
title: "Replit"
description: "Adicione assinaturas eletrônicas legalmente vinculantes a qualquer app Replit usando rotas de backend, Replit Secrets e Replit Agent."
---

O Firma permite adicionar assinaturas eletrônicas legalmente vinculantes to any Replit application. Use Replit's Node.js or Python backend to call the Firma API, manage templates, send signing requests, and track completions via webhooks.
...
### Passo 1: Store your API key as a Replit Secret
...
<Warning>
  Nunca exponha sua chave de API in frontend code. Sempre chame a API do Firma a partir de um backend route where secrets are kept secure.
</Warning>
...
### Passo 2: Create a backend route to send signing requests
...
### Passo 3: Connect the route to your app UI
...
4. Firma sends events for key state changes — see the [webhooks guide](/guides/webhooks) for all event types
...
Firma provides a [Docs MCP server](/guides/mcp) you can connect to Replit Agent. Once connected, Agent can search the Firma docs while generating code, so it writes integrations with the correct endpoints, parameters, and auth headers instead of guessing.
