---
title: "Trae"
description: "Adicione assinaturas eletrônicas legalmente vinculantes a apps que você constrói no Trae usando os servidores MCP do Firma e uma função de backend."
---

O Firma permite adicionar assinaturas eletrônicas legalmente vinculantes to any app you build in Trae, ByteDance's AI-native IDE. Because Trae has a native MCP marketplace and multi-agent collaboration, the cleanest integration is to connect Firma's MCP servers so Trae's agents can generate accurate Firma code on demand.
...
### Passo 1: Add the Firma MCP servers to Trae
...
Make sure `.env` is in `.gitignore`. Nunca exponha sua chave de API in frontend code. Sempre chame a API do Firma a partir de um backend function where secrets are kept secure.

### Passo 3: Ask Trae's agent to generate the integration
...
### Passo 4: Wire the endpoint to your app UI
...
### Passo 2: Create the backend endpoint
...
- **Resposta:** `first_signer.id` (the signing request user ID) and `first_signer.signing_link`
