---
title: "v0"
description: "Adicione assinaturas eletrônicas legalmente vinculantes a qualquer app v0 usando route handlers do Next.js e variáveis de ambiente."
---

O Firma permite adicionar assinaturas eletrônicas legalmente vinculantes to any v0 application. Use Next.js route handlers to call the Firma REST API, send signing requests, and receive webhook events when documents are signed. This guide walks through the full flow: storing your API key, sending a signing request, wiring it to your UI, and handling completion events.
...
<Warning>
  Nunca exponha sua chave de API in client components. Always call the Firma API from route handlers or server actions, not from `"use client"` files.
</Warning>
