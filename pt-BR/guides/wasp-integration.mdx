...
description: "Adicione assinaturas eletrônicas legalmente vinculantes a qualquer aplicação Wasp usando ações do lado do servidor e a API REST do Firma."
---

O Firma permite adicionar assinaturas eletrônicas legalmente vinculantes to any application built with [Wasp](https://wasp-lang.dev). Since Wasp generates a full-stack React + Node.js app with built-in auth, database (Prisma), and server actions, the integration is straightforward: create a Wasp action that calls the Firma API, and invoke it from your React frontend.
...
<Warning>
  Nunca exponha sua chave de API in client-side code. Wasp actions run on the server by default, so your key stays safe as long as you only use it inside actions.
</Warning>
