---
title: "Журнал змін API"
description: "Повний журнал змін усіх версій API та їхніх модифікацій"
icon: "clock-rotate-left"
---

Цей документ містить детальний журнал змін усіх версій Firma API, документуючи нові функції, критичні зміни та покращення між кожним випуском.

## Огляд версій

| Версія | Тип випуску | Основні зміни |
|---------|--------------|-------------|
...
Без критичних змін. These are documentation-only additions describing fields the API already returns. Existing integrations are unaffected. If you parse settings objects, ви можете now rely on the documented `show_qr_code`, `allow_presigning_download`, `identity_editable_fields`, and `notify_identity_change_email` fields, and on `emails_sent` / `recipients_failed` in cancel and resend responses.

---

## v1.26.0

**Тип випуску:** Мінорний реліз
...
```json
{
  "error": "Наступні provided fields do not match any field in the template and cannot be applied: full_name_x. Each provided field must reference an existing template field by 'template_field_id' or 'variable_name'.",
...
Без критичних змін. This release is additive, and no action is required for existing integrations.
...
Без критичних змін. The `400 "Cannot delete primary domain"` and "cannot delete the only domain" responses are removed from both `DELETE` domain endpoints; they now return `200` in those cases. Integrations that special-cased that `400` can drop the handling.
...
Без критичних змін. The new `DELETE` method on `/signing-requests/{id}` is purely additive. Existing integrations are unaffected.
...
Без критичних змін. Existing workspace webhook configurations are unaffected. Use `PATCH /workspaces/{id}` with `webhook_enabled: true` to enable workspace webhooks via API instead of the dashboard.
...
Emails are sent from `{email_local_part}@{your-custom-domain}`. Наприклад, setting `email_local_part` to `"noreply"` sends emails from `noreply@yourdomain.com` instead of `support@yourdomain.com`.
...
Без критичних змін. Existing configurations continue to send emails from `support@{domain}` by default. Set `email_local_part` at the company or workspace level to customize the sender address.
...
Без критичних змін. Existing signing requests and templates continue to work as before (auto-scroll enabled by default). To disable auto-scroll, set `disable_guided_navigation: true` at the desired level.
...
Без критичних змін. New features are opt-in:
...
Це доповнення без критичних змін. Існуючі інтеграції продовжують працювати без модифікацій.
...
**Ендпоінт:** `GET /signing-requests/{id}/download`
...
**Ендпоінт:** `PUT /workspace/{workspace_id}/settings`
...
The `create-and-send` endpoint's `fields` schema was missing several field types that were already supported by the backend and available via `anchor_tags`. Наступні types have been added to the `fields.type` enum:
...
- If you create fields via the API, ви можете now use `"stamp"` as a field type. Stamp fields should be set as `read_only: true` with the stamp image as a PNG data URL in `read_only_value`.
...
When creating a signing request from a template via `POST /signing-requests/create-and-send`, ви можете now use `variable_defined_name` to target fields for overrides instead of the internal `variable_name`:
...
Дивіться [MCP Integration guide](mcp) for setup instructions and usage examples.
...
**Ендпоінт:** `GET /signing-requests/{id}/audit`
...
Без критичних змін. All new features are additive:
...
A new endpoint allows replacing a template's PDF document while preserving all field placements. This is useful when вам потрібно update a document (e.g., fixing a typo) without recreating all the field configurations.

**Ендпоінт:** `POST /templates/{id}/replace-document`
...
Без критичних змін. The `require_otp_verification` field defaults to `null` (inherit), so existing behavior is unchanged. The replace document endpoint is purely additive.
...
| `DELETE /workspace/{workspace_id}/email-templates/{email_type}` | Видаліть workspace email template |
...
| `DELETE /company/email-templates/{email_type}` | Видаліть company email template |
...
Без критичних змін. The `hand_drawn_only` setting defaults to `false`, preserving existing behavior. Deprecated top-level integer fields are purely additive.
...
Без критичних змін. Both old and new field type names are accepted. The split PDF download fields are purely additive.
...
Без критичних змін. Email validation warnings are purely additive and do not affect existing integrations.
...
When creating signing requests from templates, ви можете now use `template_user_id` to explicitly match recipients to template users:
...
- `POST /company/domains` - Додайте новий email domain
- `GET /company/domains/{id}` - Get domain details
- `DELETE /company/domains/{id}` - Видаліть domain
...
- `POST /templates` - Створіть новий template with base64-encoded PDF
...
The initial release of the Firma Partner API with наступні core features:
