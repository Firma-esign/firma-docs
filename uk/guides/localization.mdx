...
Ви можете control the language of outgoing signing notification emails via the API by setting a `language` field at the company or workspace level.
...
Each level inherits from the one above it unless explicitly overridden. Наприклад, if you set the company language to Spanish and don't set anything on the workspace, all outgoing emails for that workspace will be sent in Spanish.
...
Ви можете force a specific language by appending the `lang` query parameter to any Firma URL:
...
### Приклад: Embed signing in Spanish
...
### Приклад: Dynamic language selection
