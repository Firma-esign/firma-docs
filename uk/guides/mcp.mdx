...
description: "Підключіть Firma до AI-помічників, таких як Claude, ChatGPT, Cursor, GitHub Copilot та інших, за допомогою Model Context Protocol (MCP)"
---
...
- **Інші MCP-клієнти** - Any client implementing the [Streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http)
...
Під час першого використання вам буде запропоновано увійти у ваш обліковий запис Firma.
...
Перезапустіть Claude Desktop після збереження. Під час першого використання вам буде запропоновано увійти у ваш обліковий запис Firma.
...
1. Натисніть **Add Connector**
2. Введіть the server URL: `https://mcp.firma.dev/mcp`
...
1. Натисніть **Create** to add a new connector
2. Введіть the server URL: `https://mcp.firma.dev/mcp`
3. Complete the OAuth sign-in flow when prompted
...
**Підтримувані клієнти:** Any client that supports the Data MCP server also supports the Docs MCP server. The setup is the same, just with a different URL.
...
1. Натисніть **Add Connector**
2. Введіть the server URL: `https://docs.firma.dev/mcp`
...
## Data MCP: доступні інструменти

Once connected, the AI assistant can use наступні tools. There are 84 tools organized across 10 categories.
...
| `signing_requests_create` | Створіть новий signing request from a document or template |
| `signing_requests_create_and_send` | Створітьnd send a signing request in one step |
...
Створітьnd manage reusable document templates.

| Tool | Description |
|------|-------------|
| `templates_list` | List templates with filtering and pagination |
| `templates_create` | Створіть новий template from a base64-encoded document |
...
| `templates_delete` | Видаліть template |
...
| `workspaces_create` | Створіть новий workspace |
...
| `webhooks_create` | Створіть новий webhook |
| `webhooks_get` | Get details of a specific webhook |
| `webhooks_update` | Оновіть webhook |
| `webhooks_delete` | Видаліть webhook |
...
| `workspace_email_templates_update` | Оновіть workspace email template |
...
| `company_email_templates_update` | Оновіть company email template |
...
| `workspace_custom_fields_create` | Створіть workspace custom field |
| `workspace_custom_fields_update` | Оновіть workspace custom field |
| `workspace_custom_fields_delete` | Видаліть workspace custom field |
| `templates_custom_fields_list` | List template custom fields |
| `templates_custom_fields_create` | Створіть template custom field |
| `templates_custom_fields_delete` | Видаліть template custom field |
| `signing_requests_custom_fields_list` | List signing request custom fields |
| `signing_requests_custom_fields_create` | Створіть signing request custom field |
| `signing_requests_custom_fields_delete` | Видаліть signing request custom field |
...
After signing in to the Data MCP, you start without a workspace selected. Ви можете either:
...
### Створітьnd send a signing request
...
- **Cursor / VS Code**: Перезапустіть застосунок after adding the configuration
