---
title: "Mocha"
description: "Додайте юридично зобов’язуючі електронні підписи до any Mocha app using AI-generated backend functions and the Firma API."
---

Firma дозволяє додавати юридично зобов’язуючі електронні підписи to any Mocha application. Store your Firma API key as a secret, ask Mocha's AI to call the API from your backend, and your users can send signing requests directly from your app.

Цей посібник охоплює how to integrate Firma using Mocha's built-in backend functions and secrets manager. Since Mocha is AI-driven, most of the integration can be done by prompting the AI with the right instructions.
...
3. Додайте новий secret with the name `FIRMA_API_KEY` and paste your Firma API key as the value
...
Mocha generates backend code from natural language prompts. You don't need to write code yourself. Instead, paste наступні prompt into the Mocha chat:
...
Replace `YOUR_TEMPLATE_ID` with the ID of a template from your Firma dashboard. Ви можете find it under **Templates** by clicking on a template and copying the ID from the URL.
...
<Note>
  The `create-and-send` endpoint creates the signing request and sends it to recipients in one call. If вам потрібно review or modify the request before sending, ask Mocha to use `POST /signing-requests` to create a draft first, then `POST /signing-requests/{id}/send` separately.
</Note>
...
Дивіться [webhooks guide](/guides/webhooks) for all event types and payload details.
...
Дивіться [embedded signing guide](/guides/embeddable-signing) для повних інструкцій налаштування and security best practices.
