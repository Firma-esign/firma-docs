...
Ви можете now customize the button text in the signing view per workspace and per language. Rename "Finish" to "Confirm", "Decline to Sign" to "Reject", or any other label that fits your workflow.
...
Дивіться [API changelog](api-changelog#v1-26-0) for the `signing_button_label_overrides` field on workspace settings.
...
Ви можете now theme the embedded template and signing-request editors with your own brand colors, not just the signing experience. Five new color controls join the existing palette so the editor chrome your users see on your pages matches your brand.
...
Дивіться [API changelog](api-changelog#v1-25-0) for the new `color_*` fields and the 11-key resolved palette.
...
Ви можете now show signers your own terms and conditions before they sign, instead of the default Firma consent statement. Set a custom statement (and an optional link to your full terms) at the company or workspace level, in any of the supported languages.
...
Дивіться [API changelog](api-changelog#v1-23-0) for the Signer Terms and company settings endpoints.
...
Ви можете now customize the local-part (before the @) of outgoing signing notification emails when using a verified custom domain. By default, emails are sent from `support@yourdomain.com`. Change it to any valid local-part, such as `noreply`, `signatures`, or `documents`.
...
Дивіться [API changelog](api-changelog#v1-21-0) for schema details.
...
Ви можете now disable the automatic scrolling behavior that guides signers to the next required field. When disabled, signers can navigate the document freely without the view automatically scrolling after completing each field. The "Next Required Field" button remains available for manual navigation.
...
Дивіться [API changelog](api-changelog#v1-20-0) for schema details.

### Company & Workspace Logo Upload

Ви можете now upload a custom logo for your company or individual workspaces directly from the Settings page. Logos appear on signing certificates and in signing notification emails.
...
Дивіться [API changelog](api-changelog#v1-19-0) for the new logo management endpoints.
...
- **Contextual field labels** - a floating indicator above the active field shows what action is needed (e.g., "Sign here", "Type text", "Виберіть a date", "Upload file")
- **Auto-advance** - after completing a field, the view automatically moves to the next required field after a brief delay
- **Keyboard navigation** - signers can Tab through fields in reading order and press Введіть to advance, enabling a fully keyboard-driven signing experience
...
Дивіться [API changelog](api-changelog#v1-18-0) for schema details.
...
Prefilled data fields can now be optionally editable by signers. Previously, fields populated from recipient data (like name, email, or custom fields) were always locked. Ви можете now allow signers to modify these values during signing, useful when prefilled data serves as a default that the signer may need to correct.
...
Ви можете now set an explicit `value` on fields when creating signing requests via the API. This lets you pre-populate any field with a specific value, independent of prefilled data or read-only settings. When both an explicit value and prefilled data are set, the explicit value takes precedence.
...
Дивіться [API changelog](api-changelog#v1-18-0) for the `prefilled_editable` property and field value details.
...
Ви можете now download a snapshot PDF of signing requests that are still in progress. From the signing request detail panel, click the download button to get a PDF showing all fields completed so far, with an audit trail page listing each signer's status.
...
Дивіться [API changelog](api-changelog#v1-16-0) for the corresponding `GET /signing-requests/{id}/download` endpoint.
...
Дивіться [API changelog](api-changelog#v01-15-00) for endpoint and schema details.
...
Ви можете now assign three distinct roles to recipients in signing requests and templates:
...
Дивіться [API changelog](api-changelog#v01-14-00) for schema details.
...
Дивіться [API changelog](api-changelog#v01-13-00) for field type schema details.
...
Firma now supports Russian and Polish across the platform. Ви можете set either language at the company, workspace, or user level. Signing flow emails, the signing interface, and the main application UI are all available in both languages.

### Referral Codes

Ви можете now share referral codes from your dashboard. When someone signs up using your referral code, both you and the new user receive bonus credits. Find your referral code and sharing link in the new referral card on the dashboard.

### Custom Field Variable Names in the API

API responses for template and signing request fields now include a `variable_defined_name` property — the human-readable name you defined when creating a custom field. Ви можете also use `variable_defined_name` to target fields when creating signing requests from templates. Дивіться [API changelog](api-changelog#v01-12-01) for details.
...
Ви можете now connect Firma to AI assistants like Claude using the Model Context Protocol (MCP). Once connected, ви можете manage templates, create and send signing requests, configure workspaces, and more through natural language conversations.
...
Дивіться [MCP Integration guide](mcp) for setup instructions and the [API changelog](api-changelog#v01-12-00) for details.
...
Дивіться [API changelog](api-changelog#v01-11-00) for the `FileFormatRules` schema and field configuration details.
...
Дивіться [API changelog](api-changelog#v01-11-00) for the full `AnchorTag` schema and properties.
...
Дивіться [API changelog](api-changelog#v01-09-00) for the `require_otp_verification` setting details.
...
Дивіться [API changelog](api-changelog#v01-09-00) for the `POST /templates/{id}/replace-document` endpoint.
...
Ви можете now require hand-drawn signatures via the API by setting `hand_drawn_only: true` in your signing request settings. When enabled, signers cannot use typed or font-based signatures and must draw their signature directly. Дивіться [API changelog](api-changelog#v01-07-00) for details.
...
- **Close button** — Enable `showCloseButton` to display a close button in the editor header. When clicked, your `onClose` callback receives `{ hasUnsavedChanges: boolean }` so ви можете prompt users to save before navigating away.
...
These options are available on both `FirmaTemplateEditor` and `FirmaSigningRequestEditor`. Дивіться [embeddable template editor](embeddable-template-editor) and [embeddable signing request editor](embeddable-signing-request-editor) guides for full details.
