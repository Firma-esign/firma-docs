---
title: "Salesforce"
description: "Додайте юридично зобов’язуючі електронні підписи до any Salesforce org using Flow, Apex, or Agentforce."
---
...
1. **Flow with External Services** — No-code. Register Firma's OpenAPI spec once, then invoke `Створітьnd send signing request` from any Flow. Best for admins.
...
3. Натисніть into the new External Credential, scroll to **Principals**, and click **New**:
...
### Крок 1: Register Firma as an External Service

1. In Setup, search for **External Services** and click **Add an External Service**
2. Виберіть **From API Specification**, then **Save and Next**
3. Configure:
   - External Service Name: `Firma`
   - Виберіть a Named Credential: `Firma_API`
...
5. Натисніть **Save and Next**
6. Виберіть the operations you want available in Flow. Common picks:
   - `POST /signing-requests/create-and-send`
   - `POST /signing-requests`
   - `POST /signing-requests/{id}/send`
   - `GET /signing-requests/{id}`
   - `GET /templates`
7. Натисніть **Save and Next**, then **Finish**
...
### Крок 2: Build the Flow

1. In Setup, open **Flows** and create a new Flow (Record-Triggered is the most common)
2. Виберіть your trigger object (Opportunity, Contract, Quote, or a custom object) and the condition that should send a signing request (e.g., `StageName` equals `Closed Won`)
3. Add an **Action** element
4. Search for `Firma` and pick `Створітьnd send signing request`
5. Map the inputs:
...
3. Натисніть **New Action** and choose **Flow** or **Apex**
   - **Flow**: pick a screen Flow or autolaunched Flow that wraps the `Firma.Створітьnd send signing request` External Services action
...
## Інтеграція webhook: react when documents are signed
...
### Крок 1: Create the Apex REST endpoint
...
### Крок 2: Verify the webhook signature
...
### Крок 3: Expose the endpoint publicly

Firma's webhook calls aren't authenticated as a Salesforce user, so expose the endpoint through an **Experience Cloud Site**. Створіть site (or use an existing one), then grant the site's guest user profile permission to execute the `FirmaWebhook` Apex class. Дивіться [Salesforce: Allow Guest Users to Access Apex REST](https://help.salesforce.com/s/articleView?id=sf.networks_guest_user_access_apex_rest.htm) for the full setup steps.

### Крок 4: Register the webhook in Firma

1. In the Firma dashboard, go to **Settings > Webhooks**
2. Додайте новий webhook pointing at `https://<your-site-domain>/services/apexrest/firma-webhook`
...
Дивіться [webhooks guide](/guides/webhooks) for all event types and payload shapes.

## Вбудований підпис in Lightning
...
Add `https://app.firma.dev` to your CSP Trusted Sites so the iframe loads. Дивіться [embedded signing guide](/guides/embeddable-signing) for full setup including security best practices.
