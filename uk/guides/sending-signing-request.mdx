...
Цей посібник охоплює creating a signing request, attaching a template or document, and inviting recipients to sign.

## Steps

1. Create or select a template
2. Створіть signing request referencing the template
...
## Створіть signing request from a template (API)

> Ендпоінт: POST /signing-requests
...
## Створіть signing request (server example) — Node (fetch)
...
## Створіть signing request (server example) — Python (requests)
...
When creating a signing request directly (POST `/signing-requests`) or updating one, ви можете add form fields:
...
Before a signing request is sent, ви можете update its details using the API. The API provides two methods:
...
**Important**: Cannot update both properties AND a recipient in the same request. Виберіть one:
...
### Приклад — fetch signing details and render iframe
