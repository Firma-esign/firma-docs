---
title: "Supabase"
description: "Додайте юридично зобов’язуючі електронні підписи до any Supabase application using Edge Functions, Postgres, and webhooks."
---
...
## Step 2: Створіть table to track signing requests
...
## Step 3: Створітьn Edge Function to send signing requests
...
Deno.serve(async (req) => {
  ...
  // Створіть signing request via Firma API
...
Створітьnother Edge Function to receive webhook events from Firma when documents are signed, declined, or updated:
...
<Warning>
  For production use, verify the webhook signature using your Firma webhook signing secret. Дивіться [webhooks guide](/guides/webhooks) for details on signature verification.
</Warning>
