---
title: "v0"
description: "Додайте юридично зобов’язуючі електронні підписи до any v0 app using Next.js route handlers and environment variables."
---

Firma дозволяє додавати юридично зобов’язуючі електронні підписи to any v0 application. Use Next.js route handlers to call the Firma REST API, send signing requests, and receive webhook events when documents are signed. This guide walks through the full flow: storing your API key, sending a signing request, wiring it to your UI, and handling completion events.
...
## Step 2: Створіть route handler to send signing requests
...
Ви можете also ask v0's chat to wire this up: *"When the user submits the contract form, call `/api/send-signing-request` with the template ID, signer email, first name, and last name from the form."* v0 will generate the form, the button, and the fetch call together.
...
1. Натисніть **Publish** in the top right of the v0 interface, or connect an existing Vercel project from **Project settings**
