...
Firma provides comprehensive white-labeling capabilities that let you create a fully branded e-signature experience. From custom logos and color themes to embedded interfaces, ви можете ensure your customers interact with your brand throughout the entire signing process.
...
Дивіться [Workspace Settings API Reference](/api-reference/v01.27.00/workspace-settings/get-workspace-settings) for the full list of configurable settings.
...
When `require_terms_acceptance` is enabled (the default), signers see a terms acceptance checkbox before they can sign. Ви можете customize the statement text and optionally link to a full terms document.
...
Дивіться [Signer Terms API Reference](/api-reference/v01.27.00/signer-terms/list-company-signer-terms) for complete endpoint documentation.
...
#### Крок 2: Add TXT verification record
...
#### Крок 3: Verify domain ownership
...
```json
{
  "message": "Domain finalized. Add наступні DNS records to enable email sending.",
  "domain": {
    "id": "123e4567-e89b-12d3-a456-426614174000",
    "domain": "acme.com",
...
For multi-tenant SaaS applications, ви можете configure different email domains per workspace. Workspace domains override the company-level domain.
...
Дивіться [Email Domains API Reference](/api-reference/v01.27.00/email-domains/list-company-domains) for complete endpoint documentation.
...
Наприклад, if you set `email_local_part` to `noreply` and your verified domain is `sign.acmecorp.com`, emails will be sent from:
...
Customize the notification emails Firma sends on your behalf so they match your brand voice and style. Ви можете set templates at both the company and workspace level, with workspace templates overriding company defaults.
...
Ви можете also retrieve this list programmatically:
...
Дивіться [Email Templates API Reference](/api-reference/v01.27.00/email-templates/list-workspace-email-templates) for complete endpoint documentation.

---

## Disabling Firma emails

For complete control over customer communications, ви можете disable Firma's automatic emails per signing request. This allows you to:
...
for (const user of users) {
  ...
  await yourEmailService.send({
    ...
    body: `Натисніть here to sign: ${signingUrl}`
  })
}
```
